function [Fig,Body_Geo,plotData] = Calc_Body_Geometry_Dec2018(XFLR5_file,STL_files,Geo_tier,...
...
for i = 1:length(movelist)
    if check_empy(i) == 0
        if i==1
            movelist_mod{i} = movelist{2}*0.001;
        else
            check_low = movelist{i-1}(1,1);
            check_high = movelist{i+i}(1,1);
            factor = check_low/check_high;
            movelist_mod{i} = movelist{i+1}*factor;
