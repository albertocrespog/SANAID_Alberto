...
%     index_vee_cy = 7; %
    ...
    if datos_sin_diedro == 1
        ...
        if index_vee_cy ~= 0
            ...
            CY_max_w2_CR = max(DATA_Ae(index_vee_cy).CY);
            
            beta_w2_CR_ope  = interp1(DATA_Ae(index_vee_cy).CY,DATA_Ae(index_vee_cy).beta,CY_max_w2_CR/Flight_SF^2,'spline');
            CY_w2_CR_ope = interp1(DATA_Ae(index_vee_cy).beta,DATA_Ae(index_vee_cy).CY,beta_w2_CR_ope,'spline');
%             CL_w2_CR_ope = interp1(DATA_Ae(index_w2).alpha,DATA_Ae(index_w2).CL*conv_w2,alpha_w2_CR_ope,'spline');

            CYbeta_vee = (CY_w2_CR_ope - DATA_Ae(index_vee_cy).CY(DATA_Ae(index_vee_cy).beta==0))/...
                (beta_w2_CR_ope - DATA_Ae(index_vee_cy).beta(DATA_Ae(index_vee_cy).beta==0))*180/pi;
