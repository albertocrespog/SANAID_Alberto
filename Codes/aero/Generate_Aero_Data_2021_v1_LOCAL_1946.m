...
if Vee == 1
    ...
    %% Determination of Stall conditions
    CL_max_w2_ope = CL_max_w2_CR/(Flight_SF^2);
    V_stall_w2 = sqrt((2*m_TOW*g)/(rho*S_ref*CL_max_w2_ope));
    V_min_w2 = Flight_SF*V_stall_w2;
    
    alpha_max_w2_ope = interp1(DATA_Ae(index_w2).CL,DATA_Ae(index_w2).alpha,CL_max_w2_ope,'spline');
    % Defines limits on CL for all aerodynamic surfaces
    CL_w2_limit_min = m_TOW*g/(0.5*rho*(V_max^2)*S_w2);
    CL_w2_limit_max = m_TOW*g/(0.5*rho*(V_min_w2^2)*S_w2);
    ...
    K_nacareport = get_k_823(Geo_tier.AR_w2_e,Geo_tier.lambda_w2);
    [KVB, KBV] = get_Vtail_body_interference(Body_Geo, Geo_tier.x_w2_LE, Geo_tier.b_w2_e);
    
%     datos_sin_diedro = 1; % proyectado si proviene de datos de wind tunnel o xflr5 con diedro datos_sin_diedro = 0;
...
%         %en Help
%         CLalpha_vee = CL_alpha_wb_Vee_0*cos((Geo_tier.dihedral_w2))^2*(Geo_tier.S_w2_e/Geo_tier.S_w2)*(KVB + KBV); %multiplicado por cos(diedro)^2 y con la interferencia del fuselaje.
%         CYbeta_vee = -K_nacareport*CL_alpha_wb_Vee_0*sin((Geo_tier.dihedral_w2))^2*(Geo_tier.S_w2_e/Geo_tier.S_w2)*(KVB + KBV);%multiplicado por sin(diedro)^2 y con la interferencia del fuselaje.
%         
%     else
%         CLalpha_vee = Aero.CL_alpha_w2_CR*(Geo_tier.S_w2_e/Geo_tier.S_w2)*(KVB + KBV); %interferencia del fuselaje.
%         CYbeta_vee = -K_nacareport*CLalpha_vee*tan((Geo_tier.dihedral_w2))^2; %o windtunnel directamente
...
%     Aero.CYbeta_vee = CYbeta_vee;
    ...
    if datos_sin_diedro == 1
        ...
        if index_vee_cy ~= 0
            CLalpha_vee = Aero.CL_alpha_w2_CR;
            %              DATA_Ae(index_w1).CL(DATA_Ae(index_w1).alpha==alpha_selected_w1
            CY_max_w2_CR = max(DATA_Ae(index_vee_cy).CY*conv_w2);
            
            beta_w2_CR_ope  = interp1(DATA_Ae(index_vee_cy).CY*conv_w2,DATA_Ae(index_vee_cy).beta,CY_max_w2_CR/Flight_SF^2,'spline');
            CY_w2_CR_ope = interp1(DATA_Ae(index_vee_cy).beta,DATA_Ae(index_vee_cy).CY*conv_w2,beta_w2_CR_ope,'spline');
%             CL_w2_CR_ope = interp1(DATA_Ae(index_w2).alpha,DATA_Ae(index_w2).CL*conv_w2,alpha_w2_CR_ope,'spline');

            CYbeta_vee = (CY_w2_CR_ope - DATA_Ae(index_vee_cy).CY(DATA_Ae(index_vee_cy).beta==0)*conv_w2)/...
                (beta_w2_CR_ope - DATA_Ae(index_vee_cy).beta(DATA_Ae(index_vee_cy).beta==0)*conv_w2)*180/pi;
            
            [CY_max_Vee_CR,max_index] = max(DATA_Ae(index_vee_cy).CY*conv_w2);
            [CY_min_Vee_CR,min_index] = min(DATA_Ae(index_vee_cy).CY*conv_w2);
            
            beta_max_Vee_CR = DATA_Ae(index_vee_cy).beta(max_index);
            beta_min_Vee_CR = DATA_Ae(index_vee_cy).beta(min_index);
            % CLmax
            CYbeta_vee2 = -(CY_max_Vee_CR -CY_min_Vee_CR)*conv_w2/...
            ...
            CLalpha_vee = Aero.CL_alpha_w2_CR;
            CYbeta_vee = -K_nacareport*CLalpha_vee*tan(Geo_tier.dihedral_w2)^2;            
        end
    end
    Aero.CLalpha_vee = CLalpha_vee;
    Aero.CYbeta_vee = CYbeta_vee;
...
%     CL_w1_limit_min
%     pause
    PL_w2 = polyfit(DATA_Ae(index_w2).CL(DATA_Ae(index_w2).CL>= CL_w1_limit_min & DATA_Ae(index_w2).CL<=CL_w1_limit_max)*conv_w2,...
        DATA_Ae(index_w2).CD(DATA_Ae(index_w2).CL>=CL_w1_limit_min & DATA_Ae(index_w2).CL<= CL_w1_limit_max)*conv_w2,2);
%     PL_w2 = PL_w1;
    Aero.PL_w2 = PL_w2;
    CD0_w2 = PL_w2(3);
    CD1_w2 = PL_w2(2);
...
if VTP == 1
    ...
    [CY_max_VTP_CR,max_index] = max(DATA_Ae(index_VTP).CY*conv_VTP);
    [CY_min_VTP_CR,min_index] = min(DATA_Ae(index_VTP).CY*conv_VTP);
    ...
    CY_0_VTP_CR = interp1(DATA_Ae(index_VTP).beta,DATA_Ae(index_VTP).CY*conv_VTP,0,'spline');
