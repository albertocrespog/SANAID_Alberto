function  [Aero_TH,Aero] = Polar_Literatura_v1(Performance_preliminar,Geo_tier,conv_UNITS,Body_Geo,AC_CONFIGURATION,OUTPUT_read_XLSX,Aero,conditions)
...
if Conf.h == 1
    ...
    wingspan2bodydiam = b_w2/w_Area_b_max;
    %%%%%%%%%%%%%%%
    % Wing 2
    % Dimensions
    % Percentage of laminar and turbulent
    ...
    if fuse_aero_FLOW_and_CBM == 1
        if wingspan2bodydiam <= 2
            CD0_w2_fuse = (S_w2_e/S_ref)*Aero.CD0_w2;
            CD1_w2_fuse = (S_w2_e/S_ref)*Aero.CD1_w2;
            CD2_w2_fuse = (S_w2_e/S_ref)*Aero.CD2_w2;
        else
            CD0_w2_fuse = (S_w2/S_ref)*Aero.CD0_w2;
            CD1_w2_fuse = (S_w2/S_ref)*Aero.CD1_w2;
            CD2_w2_fuse = (S_w2/S_ref)*Aero.CD2_w2;
        end
...
if Conf.vtail == 1
    ...
    Qc_vtail = 1.03;
    S_wet_vtail = 2*S_vee_s;

    CD0_vtail = (lam_vtail*Cf_lam_vtail + tur_vtail*Cf_turb_vtail)*FF_vtail*Qc_vtail*S_wet_vtail/S_ref;
    Aero_TH.CD0_vtail = CD0_vtail;
    
    e_vtail = 1.78*(1-0.045*AR_vee_e^(0.68)) - 0.64;
    K_vtail = (S_vee_e/S_ref)*(1/(pi*e_vtail*AR_vee_e));
...
if Conf.vtail2 == 1
    ...
    FF_vtail = (1+ (0.6/(x_c_vtail))*t_c_vtail + 100*(t_c_vtail^4))*((1.34*M^0.18)*(cos(Lambda_max_t_vtail))^0.28);
    Qc_vtail = 1.03;
    S_wet_vtail = 2*S_vee2_s;

    CD0_vtail = (lam_vtail*Cf_lam_vtail + tur_vtail*Cf_turb_vtail)*FF_vtail*Qc_vtail*S_wet_vtail/S_ref;
    Aero_TH.CD0_vtail = CD0_vtail;
    
    e_vtail = 1.78*(1-0.045*AR_vee2_e^(0.68)) - 0.64;
    K_vtail = (S_vee2_e/S_ref)*(1/(pi*e_vtail*AR_vee2_e));
...
CD0_par = CD0_w1 + CD0_can + CD0_w2 + CD0_v + CD0_v2 + CD0_vtail+ CD0_vtail2 + CD0_fus + CD0_tb + CD0_nac + CD0_missile + CD0_pod;
...
CD1_CBM = 0;
CD2_CBM = CDi_IGE*(K_w1 + K_can + K_w2 + K_vtail);

% Actualización de los resultados de FLOW con los componentes no analizados
Aero.CD0_ac = Aero.CD0_ac + CD0_v + CD0_v2 + CD0_vtail+ CD0_vtail2 + CD0_fus + CD0_tb + CD0_nac + CD0_lnd + CD0_missile + CD0_misc + CD0_pod + CD0_lp;
...
if fuse_aero_FLOW_and_CBM == 1
    ...
    % CRUISE CONDITIONS
    CD0_par = CD0_w1_fuse + CD0_can_fuse + CD0_w2_fuse + CD0_v + CD0_v2 + CD0_vtail_fuse+ CD0_vtail2_fuse + CD0_fus + CD0_tb + CD0_nac + CD0_pod +  CD0_lnd + CD0_missile;
    CD0_misc = 0;%[-] Coef de resistencia en configuración de crucero - miscelaneos
    CD0_lp = (CD0_par + CD0_misc)*0.10; %[-] Coef de resistencia en configuración de crucero miscelanea

    % Total Drag coefficients 
    CD0 = CD0_par + CD0_misc + CD0_lp;    
    CD1 = CD1_w1_fuse + CD1_can_fuse + CD1_w2_fuse;
    CD2 = CDi_IGE*(CD2_w1_fuse + CD2_can_fuse + CD2_w2_fuse);

    % TAKE OFF
    CD0_par_TO = CD0_w1_fuse + CD0_can_fuse + CD0_w2_fuse + CD0_v + CD0_v2 + CD0_vtail_fuse+ CD0_vtail2_fuse + CD0_fus + CD0_tb + CD0_nac + CD0_pod +  CD0_lnd + CD0_missile;
    ...
    CD1_TO = CD1_w1_fuse + CD1_can_fuse + CD1_w2_fuse;
    CD2_TO = CDi_IGE*(CD2_w1_fuse + CD2_can_fuse + CD2_w2_fuse);

    % LANDING
    CD0_par_LN = CD0_w1_fuse + CD0_can_fuse + CD0_w2_fuse + CD0_v + CD0_v2 + CD0_vtail_fuse+ CD0_vtail2_fuse + CD0_fus + CD0_tb + CD0_nac + CD0_pod +  CD0_lnd + CD0_missile;
    ...
    CD1_LN = CD1_w1_fuse + CD1_can_fuse + CD1_w2_fuse;
    CD2_LN = CDi_IGE*(CD2_w1_fuse + CD2_can_fuse + CD2_w2_fuse);

else % If only CBM results is used
    CD0 = CD0_CBM;
    CD1 = CD1_CBM;
    CD2 = CD2_CBM;
...
Aero_TH.CD0_par  = CD0_par; %Si fuse_aero_FLOW_and_CBM == 1, está calculado con CD012_element_fuse, y falta CD0,1,2_w2_fuse pq es 0.
...
Aero_TH.CR.K_w2 = K_w2;
Aero_TH.CR.K_vtail = K_vtail;
