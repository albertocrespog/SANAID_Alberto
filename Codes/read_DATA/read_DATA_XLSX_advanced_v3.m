function OUTPUT_read_XLSX =  read_DATA_XLSX_advanced_v3(case_AC)

%% Pre-load variables stored in Excel
% Function that reads all the data for the different aircraft
get_add_path
% filename = '../AIRCRAFT/SANAID_AIRCRAFT_DATA_v11.xlsx';
filename = 'AIRCRAFT/SANAID_AIRCRAFT_DATA_v12.xlsx';

excel_file = 'SANAID_AIRCRAFT_DATA_v12.xlsx';
...
Sheet1 = readcell(excel_file,'Sheet','1MATLAB','Range','D3:D9');
el=1;
...
write_data_der_parts = Sheet1{el};el=el+1;
...
MATLAB_flags.write_data = write_data_der_parts;

% Gathers all the flags
OUTPUT_read_XLSX.MATLAB_flags = MATLAB_flags;

%% Studies Flags
Sheet2 = readcell(excel_file,'Sheet','2Studies','Range','D3:D34');
el=1;
...
Perfo1 = Sheet2{el};el=el+1; % Variable Mass & Speed Performance Studies - Electric
Perfo2 = Sheet2{el};el=el+1; % Variable Mass & Speed Performance Studies - Fuel
Perfo3 = Sheet2{el};el=el+1; % Performance Variable Speed - Altitude
Perfo4 = Sheet2{el};el=el+1; % Glide Performance Variable Speed - Altitude	
Perfo5 = Sheet2{el};el=el+1; % Glide Performance Variable - Altitude for MAx Glide	
Perfo6 = Sheet2{el};el=el+1; % Variable Speed Performance Studies
Perfo7 = Sheet2{el};el=el+1; % Variable Mass Performance Studies

%% Corrects to avoid errors
if PERFORMANCE_STUDY == 0
    ANALYSIS_PERFORMANCE_AP = 0;
    ANALYSIS_PERFORMANCE_AP_var = 0;
    Perfo1 = 0;
    Perfo2 = 0;
    Perfo3 = 0;
    Perfo4 = 0;
...
STABILITY_STUDY_Trim_varV_gamma = Sheet2{el};el=el+1; % Trim conditions - variation V and mass
...
%% Corrects to avoid errors
if STABILITY_STUDY == 0
    STABILITY_STUDY_Trim = 0;
    STABILITY_STUDY_Regular = 0;
    STABILITY_STUDY_Trim_varV_m0 = 0;
    STABILITY_STUDY_Trim_varV_gamma = 0;
    STABILITY_STUDY_Trim_var_XCG = 0;
    STABILITY_STUDY_Long_dyn = 0;
    STABILITY_STUDY_LatDir_dyn = 0;
    STABILITY_STUDY_Trim_lat = 0;
    STABILITY_STUDY_Trim_lat_asymmetries = 0;
    STABILITY_STUDY_Trim_lat_accelerations = 0;
    STABILITY_STUDY_Trim_lat_Trim_TAB = 0;
    STABILITY_STUDY_Turning = 0;
    STABILITY_STUDY_Stability_Derivatives_varV_m0 = 0;
    STABILITY_STUDY_Stability_Analysis_varV_m0_long = 0;
    STABILITY_STUDY_Stability_Analysis_varV_m0_lat = 0;
    STABILITY_STUDY_Stability_Analysis_dyna_impulse_long = 0;
    STABILITY_STUDY_Stability_Analysis_dyna_impulse_lat = 0;
end
...
STUDY_flags.Perfo1 = Perfo1;% Variable Mass & Speed Performance Studies - Electric
STUDY_flags.Perfo2 = Perfo2;% Variable Mass & Speed Performance Studies - Fuel
STUDY_flags.Perfo3 = Perfo3;% Variable Altitude and Velocity Performance Studies
STUDY_flags.Perfo4 = Perfo4;% Variable Altitude and Velocity Performance Studies
STUDY_flags.Perfo5 = Perfo5;% Variable Altitude Performance Studies
STUDY_flags.Perfo6 = Perfo6;% Variable Speed Performance Studies
STUDY_flags.Perfo7 = Perfo7;% Variable Mass Performance Studies
...
STUDY_flags.STABILITY_STUDY_Trim_varV_gamma = STABILITY_STUDY_Trim_varV_gamma;% Trim conditions - variation V and gamma	
...
%% Weights Data
Sheet5 = readcell(excel_file,'Sheet','5Weights','Range','D3:D47');
el=1;
...
%% Fuselage Data
Sheet6 = readcell(excel_file,'Sheet','6CAD_data','Range','D3:D27');
el=1;
...
SF_CAD_AC = Sheet6{el};el=el+1; % Defines Scaling Factor for STL files for units conversion (example mm 2 m) for the case of Aircraft
AC_original = Sheet6{el};el=el+1; % Plots Cad Original Aircraft	
AC_STL_Compare = Sheet6{el};el=el+1; % % Plots both Generated Geometry and Aircraft STL comparisso	AC_STL_Compare
AC_generated = Sheet6{el};el=el+1; % % Plots Generated Geometry alone
...
Use_Storing_DATA = Sheet6{el}; el=el+1; % Use Storing Data rather that reading again data	
...
Fuselage_flags.SF_CAD_AC = SF_CAD_AC; %
Fuselage_flags.AC_original = AC_original; %
Fuselage_flags.AC_STL_Compare = AC_STL_Compare; %
Fuselage_flags.AC_generated = AC_generated; %
...
Fuselage_flags.Use_Storing_DATA = Use_Storing_DATA; %	
...
%% Aerodynamic Data
Sheet8A = readcell(excel_file,'Sheet','8AeroA','Range','D3:D45');

el=1;

index_w1 = Sheet8A{el};el=el+1; % Selection of TXT that are used for the aerodynamic analysis (w1) - LLT
index_HTP = Sheet8A{el}; el=el+1; % Selection of TXT that are used for the aerodynamic analysis (w2) - LLT
...
index_can = Sheet8A{el}; el=el+1; % Selection of TXT that are used for the aerodynamic analysis (w3) - LLT
index_VTP = Sheet8A{el}; el=el+1; % Selection of TXT that are used for the aerodynamic analysis (vtp) - LLT

index_w1_cy = Sheet8A{el};el=el+1; % Selection of TXT that are used for the aerodynamic analysis (w1-sideslip) - LLT
index_HTP_cy = Sheet8A{el}; el=el+1; % Selection of TXT that are used for the aerodynamic analysis (w2-sideslip) - LLT
...
index_can_cy = Sheet8A{el}; el=el+1; % Selection of TXT that are used for the aerodynamic analysis (w3-sideslip) - LLT
index_VTP_cy = Sheet8A{el}; el=el+1; % Selection of TXT that are used for the aerodynamic analysis (vtp-sideslip) - LLT

index_w1_VLM = Sheet8A{el}; el=el+1; % Selection of TXT that are used for the aerodynamic analysis (w1) - VLM
index_HTP_VLM = Sheet8A{el}; el=el+1; % Selection of TXT that are used for the aerodynamic analysis (w2) - VLM
...
index_can_VLM = Sheet8A{el}; el=el+1; % Selection of TXT that are used for the aerodynamic analysis (w3) - VLM
...
i_HTP = Sheet8A{el}; el=el+1; % Selects the AoA of each surface relative to fuselage line (w2)
...
i_can = Sheet8A{el}; el=el+1; % Selects the AoA of each surface relative to fuselage line (w3)
...
airfoil_vee11 = Sheet8A{el}; el=el+1; % Airfoil for VeeTail (primary)	
airfoil_vee12 = Sheet8A{el}; el=el+1; % Airfoil for VeeTail (secondary)	
airfoil_vee21 = Sheet8A{el}; el=el+1; % Airfoil for VeeTail (primary)	
airfoil_vee22 = Sheet8A{el}; el=el+1; % Airfoil for VeeTail (secondary)	
airfoil_can1 = Sheet8A{el}; el=el+1; % Airfoil for canard (primary)	
airfoil_can2 = Sheet8A{el}; el=el+1; % Airfoil for canard (secondary)	
...
fuse_aero_FLOW_and_CBM = Sheet8A{el}; el=el+1; % Use Global Weights	flag_total_weight	
datos_sin_diedro = Sheet8A{el}; el=el+1; % Uso de NACA Report 823 para estimación de datos Vtail
FLOW5_LATERAL = Sheet8A{el}; el=el+1; % Use FLOW 5 to determine Lateral properties of aerodynamic Surfaces	

% Stores the flags
Aerodynamic_Data_flags.index_w1 = index_w1; %
Aerodynamic_Data_flags.index_HTP = index_HTP; %
Aerodynamic_Data_flags.index_vee = index_vee; %
Aerodynamic_Data_flags.index_vee2 = index_vee2; %
Aerodynamic_Data_flags.index_can = index_can; %
Aerodynamic_Data_flags.index_VTP = index_VTP; %
%
Aerodynamic_Data_flags.index_w1_cy = index_w1_cy; %
Aerodynamic_Data_flags.index_HTP_cy = index_HTP_cy; %
Aerodynamic_Data_flags.index_vee_cy = index_vee_cy; %
Aerodynamic_Data_flags.index_vee2_cy = index_vee2_cy; %
Aerodynamic_Data_flags.index_can_cy = index_can_cy; %
Aerodynamic_Data_flags.index_VTP_cy = index_VTP_cy; %
%
Aerodynamic_Data_flags.index_w1_VLM = index_w1_VLM; %
Aerodynamic_Data_flags.index_HTP_VLM = index_HTP_VLM; %
Aerodynamic_Data_flags.index_vee_VLM = index_vee_VLM; %
Aerodynamic_Data_flags.index_vee2_VLM = index_vee2_VLM; %
Aerodynamic_Data_flags.index_can_VLM = index_can_VLM; %
Aerodynamic_Data_flags.index_VTP_VLM = index_VTP_VLM; %
%
Aerodynamic_Data_flags.i_w1 = i_w1; %
Aerodynamic_Data_flags.i_HTP = i_HTP; %
Aerodynamic_Data_flags.i_vee = i_vee; %
Aerodynamic_Data_flags.i_vee2 = i_vee2; %
Aerodynamic_Data_flags.i_can = i_can; %
...
Aerodynamic_Data_flags.airfoil_vee11 = airfoil_vee11; % Airfoil for VeeTail (primary)	
Aerodynamic_Data_flags.airfoil_vee12 = airfoil_vee12; % 
Aerodynamic_Data_flags.airfoil_vee21 = airfoil_vee21; % Airfoil for VeeTail (primary)	
Aerodynamic_Data_flags.airfoil_vee22 = airfoil_vee22; % 
Aerodynamic_Data_flags.airfoil_can1 = airfoil_can1; % Airfoil for canard (primary)	
Aerodynamic_Data_flags.airfoil_can2 = airfoil_can2; % Airfoil for canard (secondary)	
...
Aerodynamic_Data_flags.datos_sin_diedro = datos_sin_diedro; %% High Lift Devices  - Leading Edge
Aerodynamic_Data_flags.FLOW5_LATERAL = FLOW5_LATERAL; %% Use FLOW 5 to determine Lateral properties of aerodynamic Surfaces

% Reduced Order structure
wing1.index = index_w1;
wing2.index = index_can;
canard.index = index_can;
HTP.index = index_HTP;
VTP.index = index_VTP;
Vtail1.index = index_vee;
Vtail2.index = index_vee2;

wing1.index_VLM = index_w1_VLM;
wing2.index_VLM = index_can_VLM;
canard.index_VLM = index_can_VLM;
HTP.index_VLM = index_HTP_VLM;
VTP.index_VLM = index_VTP_VLM;
Vtail1.index_VLM = index_vee_VLM;
Vtail2.index_VLM = index_vee2_VLM;

wing1.index_cy = index_w1_cy;
wing2.index_VLM_cy = index_can_cy;
canard.index_VLM_cy = index_can_cy;
HTP.index_VLM_cy = index_HTP_cy;
VTP.index_VLM_cy = index_VTP_cy;
Vtail1.index_VLM_cy = index_vee_cy;
Vtail2.index_VLM_cy = index_vee2_cy;
...
wing1.airfoil1 = airfoil_w1;
wing1.airfoil2 = airfoil_w2;
wing2.airfoil1 = airfoil_can1;
wing2.airfoil2 = airfoil_can2;
canard.airfoil1 = airfoil_can1;
canard.airfoil2 = airfoil_can2;
HTP.airfoil1 = airfoil_HTP1;
HTP.airfoil2 = airfoil_HTP1;
VTP.airfoil1 = airfoil_VTP1;
VTP.airfoil2 = airfoil_VTP1;
Vee.airfoil1 = airfoil_vee11;
Vee.airfoil2 = airfoil_vee12;
Vee2.airfoil1 = airfoil_vee21;
Vee2.airfoil2 = airfoil_vee22;

aero.compare_plot_aero = compare_plot_aero;
aero.read_XFLR5 = read_XFLR5;
aero.read_FLOW5 = read_FLOW5;
aero.compare_plot_aero = compare_plot_aero;
aero.polar_model = polar_model;
aero.fuse_aero_FLOW_and_CBM = fuse_aero_FLOW_and_CBM;
aero.datos_sin_diedro = datos_sin_diedro;

%% Aerodynamic Data
Sheet8B = readcell(excel_file,'Sheet','8AeroB','Range','D3:D23');
el=1;
...
%% Stability Data
Sheet9 = readcell(excel_file,'Sheet','9Stability','Range','D3:D39');
el=1;
...
gamma_min = Sheet9{el}; el=el+1; % Minimum gamma angle	
gamma_max = Sheet9{el}; el=el+1; % Maximum gamma angle	
N_gamma_var = Sheet9{el}; el=el+1; % Number of iterations for variation of gamma angle
...
Stability_flags.gamma_min = gamma_min;
Stability_flags.gamma_max = gamma_max;
Stability_flags.N_gamma_var = N_gamma_var;
...
Sheet11 = readcell(excel_file,'Sheet','11InputGeometry','Range','D3:D242');
el=1;
...
z_loc_1R_yB1_VTP_CAD = Sheet11{el}; el=el+1; % z loc of VTP kink1 chord LE position (distance from CAD refference point)	
z_loc_1R_yB2_VTP_CAD = Sheet11{el}; el=el+1; % z loc of wing (HTP) kink2 chord LE position (distance from CAD refference point)	
...
Lambda_LE_VTP_k1_e  = Sheet11{el}; el=el+1; % Sweep of kink1 (deg)	
Lambda_LE_VTP_k2_e  = Sheet11{el}; el=el+1; % Sweep of kink2 (deg)	
dihedral_VTP_e = Sheet11{el}; el=el+1; %	Dihedral of VTP (deg)
dihedral_VTP_k1_e = Sheet11{el}; el=el+1; % Dihedral of kink1 (deg)	
dihedral_VTP_k2_e = Sheet11{el}; el=el+1; % Dihedral of kink2 (deg)	
%VTAIL
y_loc_1R_y1_vee_CAD = Sheet11{el}; el=el+1; %	y loc of wing (V-tail) root chord LE position (distance from CAD refference point)
y_loc_1R_yB1_vee_CAD = Sheet11{el}; el=el+1; % y loc of wing (vee) kink1 chord LE position (distance from CAD refference point)	
y_loc_1R_yB2_vee_CAD = Sheet11{el}; el=el+1; % y loc of wing (vee) kink2 chord LE position (distance from CAD refference point)	
...
Lambda_LE_vee_k1_e = Sheet11{el}; el=el+1; % Sweep of kink1 Vee (deg)	
Lambda_LE_vee_k2_e = Sheet11{el}; el=el+1; % Sweep of kink2 Vee (dVeg)	
dihedral_vee_e = Sheet11{el}; el=el+1; %	Dihedral of V-tail (deg)
dihedral_vee_k1_e = Sheet11{el}; el=el+1; % Dihedral of kink1 Vee (deg)	
dihedral_vee_k2_e = Sheet11{el}; el=el+1; % Dihedral of kink2 Vee (deg)	
%VTAIL2
y_loc_1R_y1_vee2_CAD = Sheet11{el}; el=el+1; %	y loc of wing (V-tail) root chord LE position (distance from CAD refference point)
y_loc_1R_yB1_vee2_CAD = Sheet11{el}; el=el+1; % y loc of wing (vee) kink1 chord LE position (distance from CAD refference point)	
y_loc_1R_yB2_vee2_CAD = Sheet11{el}; el=el+1; % y loc of wing (vee) kink2 chord LE position (distance from CAD refference point)	
...
Lambda_LE_vee2_k1_e = Sheet11{el}; el=el+1; % Sweep of kink1 Vee (deg)	
Lambda_LE_vee2_k2_e = Sheet11{el}; el=el+1; % Sweep of kink2 Vee (dVeg)	
dihedral_vee2_e = Sheet11{el}; el=el+1; %	Dihedral of V-tail (deg)
dihedral_vee2_k1_e = Sheet11{el}; el=el+1; % Dihedral of kink1 Vee (deg)	
dihedral_vee2_k2_e = Sheet11{el}; el=el+1; % Dihedral of kink2 Vee (deg)	
%Twin VTP
y_loc_1R_y1_2VTP_CAD = Sheet11{el}; el=el+1; %	y loc of wing (VTP) root chord LE position (distance from CAD refference point)
z_loc_1R_y1_2VTP_CAD = Sheet11{el}; el=el+1; %	y loc of wing (VTP) tip chord LE position (distance from CAD refference point)
z_loc_1R_yB1_2VTP_CAD = Sheet11{el}; el=el+1; % z loc of VTP kink1 chord LE position (distance from CAD refference point)	
z_loc_1R_yB2_2VTP_CAD = Sheet11{el}; el=el+1; % z loc of wing (HTP) kink2 chord LE position (distance from CAD refference point)	
z_loc_1R_y2_2VTP_CAD = Sheet11{el}; el=el+1; %	z loc of wing (VTP) root chord LE position (distance from CAD refference point)
x_loc_1R_y1_2VTP_CAD = Sheet11{el}; el=el+1; %	x loc of wing (VTP) root chord LE position (distance from CAD refference point)
Lambda_LE_2VTP_e = Sheet11{el}; el=el+1; %	Sweep of VTP (deg)
Lambda_LE_2VTP_k1_e  = Sheet11{el}; el=el+1; % Sweep of kink1 (deg)	
Lambda_LE_2VTP_k2_e  = Sheet11{el}; el=el+1; % Sweep of kink2 (deg)	
dihedral_2VTP_e = Sheet11{el}; el=el+1; %	Dihedral of VTP (deg)
dihedral_2VTP_k1_e = Sheet11{el}; el=el+1; % Dihedral of kink1 (deg)	
dihedral_2VTP_k2_e = Sheet11{el}; el=el+1; % Dihedral of kink2 (deg)	
% Chord geometry
...
cB_k1_VTP = Sheet11{el}; el=el+1; %	Kink1 Chord VTP
cB_k2_VTP = Sheet11{el}; el=el+1; %	Kink2 Chord VTP	
cT_VTP = Sheet11{el}; el=el+1; %	Tip Chord VTP
%
cR_vee = Sheet11{el}; el=el+1; %	Root Chord vee
cB_k1_vee = Sheet11{el}; el=el+1; %	Kink1 Chord Vee
cB_k2_vee = Sheet11{el}; el=el+1; %	Kink2 Chord Vee	
cT_vee = Sheet11{el}; el=el+1; %	Tip Chord vee
%
cR_vee2 = Sheet11{el}; el=el+1; %	Root Chord vee2
cB_k1_vee2 = Sheet11{el}; el=el+1; %	Kink1 Chord Vee2
cB_k2_vee2 = Sheet11{el}; el=el+1; %	Kink2 Chord Vee2	
cT_vee2 = Sheet11{el}; el=el+1; %	Tip Chord vee2
%
cR_2VTP = Sheet11{el}; el=el+1; %	Root Chord Twin VTP
cB_k1_2VTP = Sheet11{el}; el=el+1; %	Kink1 Chord VTP
cB_k2_2VTP = Sheet11{el}; el=el+1; %	Kink2 Chord VTP	
...
K_y1_ele_HTP = Sheet11{el}; el=el+1; %	% of elevator from effective HTP surface - inner position
K_y2_ele_HTP = Sheet11{el}; el=el+1; %	% of elevator from effective HTP surface  - outter position
...
K_y1_rudvtr_vee = Sheet11{el}; el=el+1; %	% of ruddervator from effective V-tail surface - inner position
K_y2_rudvtr_vee= Sheet11{el}; el=el+1; %	% of ruddervator from effective V-tail surface  - outter position
K_y1_rudvtr_vee2 = Sheet11{el}; el=el+1; %	% of ruddervator from effective V-tail surface - inner position
K_y2_rudvtr_vee2 = Sheet11{el}; el=el+1; %	% of ruddervator from effective V-tail surface  - outter position
...
K_y1_TT_ele_HTP = Sheet11{el}; el=el+1;% of Trim Tab  elevator from effective HTP surface - inner position	
K_y2_TT_ele_HTP = Sheet11{el}; el=el+1;% of Trim Tab  elevator from effective HTP surface  - outter position	
...
d_rudvtr2 = Sheet11{el}; el=el+1; %Definition of available control surface - ruddervator
...
InputGeometry_Data_flags.z_loc_1R_yB1_VTP_CAD = z_loc_1R_yB1_VTP_CAD; %
InputGeometry_Data_flags.z_loc_1R_yB2_VTP_CAD = z_loc_1R_yB2_VTP_CAD; %
...
InputGeometry_Data_flags.Lambda_LE_VTP_k1_e = Lambda_LE_VTP_k1_e; %
InputGeometry_Data_flags.Lambda_LE_VTP_k2_e = Lambda_LE_VTP_k2_e; %
InputGeometry_Data_flags.dihedral_VTP_e = dihedral_VTP_e; %
InputGeometry_Data_flags.dihedral_VTP_k1_e = dihedral_VTP_k1_e; %
InputGeometry_Data_flags.dihedral_VTP_k2_e = dihedral_VTP_k2_e; %

% Vee Tail 
InputGeometry_Data_flags.y_loc_1R_y1_vee_CAD = y_loc_1R_y1_vee_CAD; %
InputGeometry_Data_flags.y_loc_1R_yB1_vee_CAD = y_loc_1R_yB1_vee_CAD; %
InputGeometry_Data_flags.y_loc_1R_yB2_vee_CAD = y_loc_1R_yB2_vee_CAD; %
...
InputGeometry_Data_flags.Lambda_LE_vee_k1_e = Lambda_LE_vee_k1_e; %
InputGeometry_Data_flags.Lambda_LE_vee_k2_e = Lambda_LE_vee_k2_e; %
InputGeometry_Data_flags.dihedral_vee_e = dihedral_vee_e; %
InputGeometry_Data_flags.dihedral_vee_k1_e = dihedral_vee_k1_e; %
InputGeometry_Data_flags.dihedral_vee_k2_e = dihedral_vee_k2_e; %

% Vee Tail 2 
InputGeometry_Data_flags.y_loc_1R_y1_vee2_CAD = y_loc_1R_y1_vee2_CAD; %
InputGeometry_Data_flags.y_loc_1R_yB1_vee2_CAD = y_loc_1R_yB1_vee2_CAD; %
InputGeometry_Data_flags.y_loc_1R_yB2_vee2_CAD = y_loc_1R_yB2_vee2_CAD; %
...
InputGeometry_Data_flags.Lambda_LE_vee2_k1_e = Lambda_LE_vee2_k1_e; %
InputGeometry_Data_flags.Lambda_LE_vee2_k2_e = Lambda_LE_vee2_k2_e; %
InputGeometry_Data_flags.dihedral_vee2_e = dihedral_vee2_e; %
InputGeometry_Data_flags.dihedral_vee2_k1_e = dihedral_vee2_k1_e; %
InputGeometry_Data_flags.dihedral_vee2_k2_e = dihedral_vee2_k2_e; %
...
InputGeometry_Data_flags.z_loc_1R_yB1_2VTP_CAD = z_loc_1R_yB1_2VTP_CAD; %
InputGeometry_Data_flags.z_loc_1R_yB2_2VTP_CAD = z_loc_1R_yB2_2VTP_CAD; %
...
InputGeometry_Data_flags.Lambda_LE_2VTP_k1_e = Lambda_LE_2VTP_k1_e; %
InputGeometry_Data_flags.Lambda_LE_2VTP_k2_e = Lambda_LE_2VTP_k2_e; %
InputGeometry_Data_flags.dihedral_2VTP_e = dihedral_2VTP_e; %
InputGeometry_Data_flags.dihedral_2VTP_k1_e = dihedral_2VTP_k1_e; %
InputGeometry_Data_flags.dihedral_2VTP_k2_e = dihedral_2VTP_k2_e; %
% 
% Chord geometry
%wing 1
...
InputGeometry_Data_flags.cB_k1_VTP = cB_k1_VTP; %
InputGeometry_Data_flags.cB_k2_VTP = cB_k2_VTP; %
InputGeometry_Data_flags.cT_VTP = cT_VTP; %
%Vee Tail
InputGeometry_Data_flags.cR_vee = cR_vee; %
InputGeometry_Data_flags.cB_k1_vee = cB_k1_vee; %
InputGeometry_Data_flags.cB_k2_vee = cB_k2_vee; %
InputGeometry_Data_flags.cT_vee = cT_vee; %
%Vee Tail2
InputGeometry_Data_flags.cR_vee2 = cR_vee2; %
InputGeometry_Data_flags.cB_k1_vee2 = cB_k1_vee2; %
InputGeometry_Data_flags.cB_k2_vee2 = cB_k2_vee2; %
InputGeometry_Data_flags.cT_vee2 = cT_vee2; %
% Twin VTP
InputGeometry_Data_flags.cR_2VTP = cR_2VTP; %
InputGeometry_Data_flags.cB_k1_2VTP = cB_k1_2VTP; %
InputGeometry_Data_flags.cB_k2_2VTP = cB_k2_2VTP; %
...
InputGeometry_Data_flags.K_y1_ele_HTP = K_y1_ele_HTP;
InputGeometry_Data_flags.K_y2_ele_HTP = K_y2_ele_HTP;
...
InputGeometry_Data_flags.K_y1_rudvtr_vee = K_y1_rudvtr_vee;
InputGeometry_Data_flags.K_y2_rudvtr_vee = K_y2_rudvtr_vee;
InputGeometry_Data_flags.K_y1_rudvtr_vee2 = K_y1_rudvtr_vee2;
InputGeometry_Data_flags.K_y2_rudvtr_vee2 = K_y2_rudvtr_vee2;
...
InputGeometry_Data_flags.K_y1_TT_ele_HTP = K_y1_TT_ele_HTP;
InputGeometry_Data_flags.K_y2_TT_ele_HTP = K_y2_TT_ele_HTP;
...
InputGeometry_Data_flags.d_rudvtr2 = d_rudvtr2; %Definition of available control surface - ruddervator
...
%% PLOTS
Sheet12 = readcell(excel_file,'Sheet','12Plots','Range','D3:D33');
el=1;
plot_aero1 = Sheet12{el}; el=el+1;% Prints plots - Aero
plot_aero2 = Sheet12{el}; el=el+1;% Prints plots - Aero Polar
plot_aero3 = Sheet12{el}; el=el+1;% Print Plots for EStimation ofXAC
plot_prop1 = Sheet12{el}; el=el+1;% Print Plots for Propulsive Models
plot_3D = Sheet12{el}; el=el+1;% Prints plots for 3D
plot_stability1 = Sheet12{el}; el=el+1;% Prints plots of SM analysis
plot_stability2 = Sheet12{el}; el=el+1;% Prints plots of longitudinal Trim
plot_stability3 = Sheet12{el}; el=el+1;% Prints plots of longitudinal Trim with Variable mass & Variable Speed
plot_stability4 = Sheet12{el}; el=el+1;% Prints plots of longitudinal Trim with Variable gamma & Variable Speed
plot_stability5 = Sheet12{el}; el=el+1;% Prints plots of lateral Trim
plot_stability6 = Sheet12{el}; el=el+1;% Prints plots of lateral Trim - Asymmetries
plot_stability7 = Sheet12{el}; el=el+1;% Prints plots of lateral Trim - Accelerations
plot_stability8 = Sheet12{el}; el=el+1;% Prints plots of lateral Trim - Trim Tabs
plot_stability9 = Sheet12{el}; el=el+1;% Prints plots of lateral Turning
plot_stability10 = Sheet12{el}; el=el+1;% Prints PLOTS STABILITY DERIVATIVES FOR VAR MASS AND VELOCITY
plot_stability11 = Sheet12{el}; el=el+1;% Prints PLOTS STABILITY ANALYSIS FOR VAR MASS AND VELOCITY
plot_stability12 = Sheet12{el}; el=el+1;% Prints PLOTS DYNAMIC STABILITY ANALYSIS AFTER IMPULSE
plot_stability13 = Sheet12{el}; el=el+1;% Prints plots of longitudinal Trim with V-n diagram n & Variable Speed
plot_perfo1 = Sheet12{el}; el=el+1;% Prints PLOTS PERFORMANCE STUDY
plot_perfo2 = Sheet12{el}; el=el+1; % Prints plots of Performance for Variable V and mass
plot_perfo3 = Sheet12{el}; el=el+1; % Prints plots of Performance for Variable h and V
plot_perfo4 = Sheet12{el}; el=el+1; % Prints plots of Performance Glide for Variable h and V
plot_perfo5 = Sheet12{el}; el=el+1; % Prints plots of Performance Max Glide for Variable h
...
% Stores the flags
PLOT_flags.plot_aero1 = plot_aero1; %
PLOT_flags.plot_aero2 = plot_aero2; %
PLOT_flags.plot_aero3 = plot_aero3; %
PLOT_flags.plot_prop1 = plot_prop1; %
PLOT_flags.plot_3D = plot_3D; %
PLOT_flags.plot_stability1 = plot_stability1; %
PLOT_flags.plot_stability2 = plot_stability2; %
PLOT_flags.plot_stability3 = plot_stability3; %
PLOT_flags.plot_stability4 = plot_stability4; %
PLOT_flags.plot_stability5 = plot_stability5; %
PLOT_flags.plot_stability6 = plot_stability6; %
PLOT_flags.plot_stability7 = plot_stability7; %
PLOT_flags.plot_stability8 = plot_stability8; %
PLOT_flags.plot_stability9 = plot_stability9; %
PLOT_flags.plot_stability10 = plot_stability10; %
PLOT_flags.plot_stability11 = plot_stability11; %
PLOT_flags.plot_stability12 = plot_stability12; %
PLOT_flags.plot_stability13 = plot_stability13; %
PLOT_flags.plot_perfo1 = plot_perfo1; %
PLOT_flags.plot_perfo2 = plot_perfo2; %
PLOT_flags.plot_perfo3 = plot_perfo3; %
PLOT_flags.plot_perfo4 = plot_perfo4; %
PLOT_flags.plot_perfo5 = plot_perfo5; %
...
n_tr_wt = Sheet13{el}; el=el+1;%	TURN WAIT: FACTOR DE CARGA
R_tr_wt = Sheet13{el}; el=el+1;%	TURN WAIT: RADIO DE GIRO (m)
% lANDING
temp_local_LND = Sheet13{el}; el=el+1;%	LANDING:  TEMPERATURA LOCAL (Celsius)
h_inicial_LND = Sheet13{el}; el=el+1;%	LANDING: ALTURA LOCAL (m)
P_local_LND = Sheet13{el}; el=el+1;%	LANDING:  PRESION LOCAL (Pa)
mu_LND = Sheet13{el}; el=el+1;%	LANDING: COEFICIENTE DE FRICCION CON LA PISTA (MU)
delta_T_LND = Sheet13{el}; el=el+1;%	LANDING:  PALANCA DE REVERSA
t_brake = Sheet13{el}; el=el+1;%	LANDING: 'Tiempo en activar frenos' - [s]
% Dummy Segment
dummy(1) = Sheet13{el}; el=el+1;%	DUMMY SEGMENT: ALTURA INICIAL (CRUISE)
dummy(2) = Sheet13{el}; el=el+1;%	DUMMY SEGMENT: DISTANCIA FINAL
dummy(3) = Sheet13{el}; el=el+1;%	DUMMY SEGMENT: VELOCIDAD (CRUISE)
