function Propulsion = get_engine_piston_custom(h,V,rho,alpha,beta,Geo_tier,Fdes,Prop_data,AC_CONFIGURATION,OUTPUT_read_XLSX) % 1: TIPO DE MOTOR -->  3_PISTON
...
% R_heli = D_prop/2;
% D = Prop_data.D_prop;
D = OUTPUT_read_XLSX.Propulsive_flags.D_prop;
...
% initialization
T_check = 0;
Delta_p = 0.1; % increment in Delta_T
delta_p = 10; % initial throttle 
...
%     prop = 3612; % Propeller
%     prop = 3412; % Propeller
    [T_calc,c,Vnmax,delta_act,n,Prop_data] = propulsive_model_SP210_SER(V,h,delta_p,prop_properties);
    T_check = T_calc;
