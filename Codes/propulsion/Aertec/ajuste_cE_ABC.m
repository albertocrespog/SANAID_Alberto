...
function [ABC]=ajuste_cE_ABC(deltae,n_ref)
...
% cálculo de C1,C2yC3, que son independientes de deltae y por tanto de F,
% que es la unica funcion de deltae

d= @(abc) n_ref.^2;
f= @(abc) (abc(1)*n.^2+abc(2)*n*n_ref+abc(3)*n_ref^2)./d(abc);
M1=@(abc) sum(((ce-f(abc)).*f(abc))./d(abc));
M2=@(abc) sum(((ce-f(abc)).*n.*f(abc))./d(abc));
M3=@(abc) sum(((ce-f(abc)).*(n.^2).*f(abc))./d(abc));
M=@(abc) [M1(abc),M2(abc),M3(abc)];
M([1,1,1])

ABC=fsolve(M,[1,-1,1]);

f=@(abc,n) (abc(1)*n.^2+abc(2)*n*n_ref+abc(3)*n_ref^2)./d(abc);
plot(n,ce,'or')
hold on
xx=16:100;
yy=f(ABC,xx);
