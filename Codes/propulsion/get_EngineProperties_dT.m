function [Propulsion] = get_EngineProperties_dT(h,V,rho,alpha,beta,Geo_tier,delta_T,Prop_data,AC_CONFIGURATION)
...
%         delta_T = (Fdes/n_eng)/(T_SL*(1+0.2*M^2)^(1.4./0.4)*(1-0.49.*sqrt(M))*(rho/rho_SL));
        ...
        dT_dV = delta_T*(1.400000000*T_SL*(1+.2*V^2/a^2)^2.500000000*(1-.49*sqrt(V/a))*rho*V/(rho_SL*a^2)-...
            .2450000000*T_SL*(1+.2*V^2/a^2)^3.500000000*rho/(sqrt(V/a)*a*rho_SL));
        dCT_dV = delta_T*(2.800000000*T_SL*(1+.2*V^2/a^2)^2.500000000*(1-.49*sqrt(V/a))/(rho_SL*V*S_ref*a^2)-...
        ...
        dT_dM =  delta_T*(1.400000000*T_SL*(1+.2*M^2)^2.500000000*(1-.49*sqrt(M))*rho*M/rho_SL-...
            .2450000000*T_SL*(1+.2*M^2)^3.500000000*rho/(sqrt(M)*rho_SL));
        dCT_dM = delta_T*(2.800000000*T_SL*(1+.2*M^2)^2.500000000*(1-.49*sqrt(M))*M/(rho_SL*V^2*S_ref)-...
            .4900000000*T_SL*(1+.2*M^2)^3.500000000/(sqrt(M)*rho_SL*V^2*S_ref));
...
%         delta_T = (Fdes/n_eng)/(T_SL*eta_p/V*(1+0.2.*M^2)^(1.4./0.4)*(rho*Temp/(rho_SL*Temp_SL)));
        ...
        dT_dM = 1.400000000*delta_T*T_SL*eta_p*(1+.2*M^2)^2.500000000*rho*Temp*M/(V*rho_SL*Temp_SL);
        dCT_dM = 2.800000000*delta_T*T_SL*eta_p*(1+.2*M^2)^2.500000000*Temp*M/(V^3*rho_SL*Temp_SL*S_ref);
        dP_dM = 1.400000000*delta_T*T_SL*eta_p*(1+.2*M^2)^2.500000000*rho*Temp*M/(rho_SL*Temp_SL);
        dCP_dM = 2.800000000*delta_T*T_SL*eta_p*(1+.2*M^2)^2.500000000*Temp*M/(rho_SL*Temp_SL*V^2*S_ref*cmac);
        % Estimation of Prop data
        A_prop = pi*(D/2)^2;
        ...
        Propulsion.d_T_d_M = dT_dM;
        Propulsion.d_CT_d_M = dCT_dM;
        Propulsion.d_P_d_M = dT_dM;
        Propulsion.d_CP_d_M = dCT_dM;
    case 3
