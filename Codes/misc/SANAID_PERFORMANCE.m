...
[Body_Geo,meshData] = Generation_Fuselage_Data_old(Geo_tier,XFLR5_DATA,ESCALADO,XFLR5_file,STL_file,OUTPUT_read_XLSX);% Defines Propulsion DATA
...
if OUTPUT_read_XLSX.STUDY_flags.PERFORMANCE_STUDY == 1
    Storing_PERFORMANCE_DATA_1 = Conduct_Performance_Optimzation_v1(Aero,Aero_TH,case_AC,conv_UNITS,OUTPUT_read_XLSX,...
...
if OUTPUT_read_XLSX.STUDY_flags.STABILITY_STUDY == 1
    ...
    if OUTPUT_read_XLSX.STUDY_flags.STABILITY_STUDY_Trim == 1
        ...
        % Saving variables
        if OUTPUT_read_XLSX.MATLAB_flags.save_MAT == 1
            Storing_STABILITY_DATA_1 = Saving_data_Stability_TRIM(TRIM_RESULTS,Trim_ITER,Stab_Der,Stab_Der_parts,Stab_Dyn_Long,Stab_Dyn_LatDir);
        end
    else
        % Stores dummy variable for continuity
        dummy = 1;
...
%     V_high = Performance.V_max;
    ...
    if OUTPUT_read_XLSX.STUDY_flags.STABILITY_STUDY_Trim_varV_m0 == 1
        ...
        for i=1:N_V_VAR
            ...
            for j=1:N_m_VAR
                ...
                [TRIM_RESULTS_calc_var_V_XCG,Trim_ITER_calc_var_V_XCG,Stab_Der_calc_var_V_XCG,Stab_Der_parts_calc_var_V_XCG] = Calculo_Stability_Derivatives_Noviembre2021_v2...
                    (conditions,Aero,Aero_TH,Geo_tier,Weight_tier,Prop_data,conv_UNITS,...
                    Body_Geo,Design_criteria,Posicion_Palanca,SM_des,XCG_FF,DATA_Ae,Performance,OUTPUT_read_XLSX,AC_CONFIGURATION,only_trim);
                TRIM_RESULTS_var_V_XCG{i,j} = TRIM_RESULTS_calc_var_V_XCG;
                Trim_ITER_var_V_XCG{i,j} = Trim_ITER_calc_var_V_XCG;
                Stab_Der_var_V_XCG{i,j} = Stab_Der_calc_var_V_XCG;
                Stab_Der_parts_V_XCG{i,j} = Stab_Der_parts_calc_var_V_XCG;
                %% Dynamic Stability Analysis
                StabilityModel = OUTPUT_read_XLSX.Stability_flags.StabilityModel;
                Variable_Study = 1;
                Stab_Dyn_Long_var_V_XCG{i,j} = longitudinal_analysis(Performance,Stab_Der_calc_var_V_XCG,conv_UNITS,StabilityModel,...
                    Weight_tier,TRIM_RESULTS_calc_var_V_XCG,Geo_tier,Variable_Study,conditions);
                Stab_Dyn_LatDir_var_V_XCG{i,j} = lateral_directional_analysis(Performance,Stab_Der_calc_var_V_XCG,conv_UNITS,StabilityModel,...
                    Weight_tier,TRIM_RESULTS_calc_var_V_XCG,Geo_tier,Variable_Study,conditions);
            end
        end
        ...
        %% Saves the data to a mat file
        if OUTPUT_read_XLSX.MATLAB_flags.save_MAT == 1
            Storing_STABILITY_DATA_2 = Saving_data_Stability_varV(TRIM_RESULTS_var_V_XCG,Trim_ITER_var_V_XCG,Stab_Der_var_V_XCG,Stab_Der_parts_V_XCG,Stab_Dyn_Long_var_V_XCG,Stab_Dyn_LatDir_var_V_XCG,case_AC);
        end
    else
        % Stores dummy variable for continuity
        dummy = 1;
    ...
    if OUTPUT_read_XLSX.STUDY_flags.STABILITY_STUDY_Trim_var_XCG
        ...
        for i=1:N_x_XCG_VAR
            ...
            conditions.m_TOW = m_TOW;
            [TRIM_RESULTS_calc_var_xcg,Trim_ITER_calc_var_xcg,Stab_Der_calc_var_xcg,Stab_Der_parts_calc_var_xcg] = Calculo_Stability_Derivatives_Noviembre2021_v2...
                (conditions,Aero,Aero_TH,Geo_tier,Weight_tier,Prop_data,conv_UNITS,...
                Body_Geo,Design_criteria,Posicion_Palanca,SM_des,XCG_FF,DATA_Ae,Performance,OUTPUT_read_XLSX,AC_CONFIGURATION,only_trim);
            ...
            Stab_Dyn_Long_var_XCG{i} = longitudinal_analysis(Performance,Stab_Der_calc_var_xcg,conv_UNITS,StabilityModel,...
                Weight_tier,TRIM_RESULTS_calc_var_xcg,Geo_tier,Variable_Study,conditions);
        ...
        %% Saves the data to a mat file
        if OUTPUT_read_XLSX.MATLAB_flags.save_MAT == 1
            Storing_STABILITY_DATA_3 = Saving_data_Stability_varXCG(TRIM_RESULTS_var_XCG,Trim_ITER_var_XCG,Stab_Der_var_XCG,Stab_Der_parts_var_XCG,Stab_Dyn_Long_var_XCG,Stab_Dyn_LatDir_var_XCG,case_AC);
        end
    else
        % Stores dummy variable for continuity
        dummy = 1;
...
GENERATE_PLOTS(OUTPUT_read_XLSX,Geo_tier,Plot_Options,Fig,Storing_STABILITY_DATA_1,Storing_STABILITY_DATA_2,Storing_STABILITY_DATA_3,...
    ...
    AC_CONFIGURATION,meshData,case_AC);
