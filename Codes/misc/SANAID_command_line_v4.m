...
% Load the data from SANAID for the Original Aircraft Configuration
load('Geo_tier.mat')
...
[Body_Geo,meshData] = Generation_Fuselage_Data_old(Geo_tier,XFLR5_DATA,CASE_fuse,ESCALADO,XFLR5_file,STL_file,SF);
...
if OUTPUT_read_XLSX.STUDY_flags.AERODYNAMIC_STUDY == 1
    ...
    switch case_AC
        ...
        case 8 % case_AC = 8 - TAMIZ
            [casos,prefix,mark_legend,X_OC] = read_aero_files_June2019_v3_EMERGENTIA(Performance_preliminar);
        case 9 % case_AC = 9 - EMERGENTIA Wind Tunnel Model
            [casos,prefix,mark_legend,X_OC] = read_aero_files_June2019_v3_EMERGENTIA(Performance_preliminar);
    end
...
%% Generates the file for Propeller Data
switch case_AC
    case 1 % case_AC = 1 - EMERGENTIA 1:1
        switch compare
    ...
    case 9 % case_AC = 9 - EMERGENTIA 1:4 Wind Tunnel Model
        switch compare
            case 1 % compare_prop = 1 compares APC Models alone
                VECTOR_Prop.v1 = [1];
            case 2 % compare_prop = 2 compares APC with wind tunnel models
...
%         Post_processing_PERFORMANCE = OUTPUT_read_XLSX.PerformanceStudy_flags.Post_processing_PERFORMANCE;
% 
%         % Creates the variable or single conditions
...
%                 if type_missions(1) == 3
%                     datos_var{i,j} = datos(1).segmento;
%                 end
%                 if type_missions(1) == 5
%                     datos_var{i,j} = datos(1).segmento;
%                 end
%                 if type_missions(1) == 8
%                     datos_var{i,j} = datos(1).segmento;
%                 end
%                 datos_var{i,j} = datos(1).segmento;
%             end
