...
% Analizes performance integrated with Academic Performance Codes
...
%% Propulsion data depending on source
    % Source type
    % Case 1 :Combustion Engine
    % Case 2 :Powered by batteries

    %Source_type=2

Propulsion = Propulsion_Data_CE(AC_CONFIGURATION);
%Propulsion = Propulsion_Data_CE(AC_CONFIGURATION,Source_type);
    ...
    if Study_Prop_Limits == 1
        ...
        if Study_vertical == 1;
            ...
            [Fig] = Get_Vertical_Flight_Limits(Aero_TH,pp,RPMMAX_APC,Prop_data,Weight_tier,conv_UNITS,Performance,Geo_tier,...
                AC_CONFIGURATION,N_Vv,Vv_vect,Fig,PLOT_Get_Vertical_Flight_Limits,solve_w_fero);
        end
        % Selects to plot Get Vertical Flight Limits that determines the
        % minimum prop diameter for the different maneuvers
        ...
        [Fig] = Get_Horizontal_Flight_Limits(Aero_TH,pp,RPMMAX_APC,Prop_data,Weight_tier,conv_UNITS,Performance,Geo_tier,...
            AC_CONFIGURATION,N_Vh,Vh_vect,Fig,PLOT_Get_Horizontal_Flight_Limits,solve_w_fero);
    end
...
%     plots_contour_sensitivity = 0; % represents contour plots
    special_PLOTS = 0; % represents the plotys searching for minimum Energy
    ...
    if PERFORMANCE_sensitivity == 1
        ...
        if sensitivity_vertical == 1;
            ...
            [X,Yv,Z_Tv,Z_Pev,Z_deltav,Z_RPMv,Z_Qv,Z_Ev,Z_ethav,Fig] = Sensitivity_Study_Vertical_Flight(Aero_TH,SF_prop,RPMMAX_APC,Prop_data,...
                Weight_tier,conv_UNITS,...
                Geo_tier,AC_CONFIGURATION,Fig,PLOTS_SENSITIVITY_VERTICAL,Performance,solve_w_fero,...
            ...
            while query_properties == 1
                ...
                switch answer_prop
                    case 'Yes'
                        ...
                        z_T = interp2(X,Yv,Z_Tv,D_propin_q,Vv_q,'cubic');
                        z_Pe = interp2(X,Yv,Z_Pev,D_propin_q,Vv_q,'cubic');
                        z_Q = interp2(X,Yv,Z_Qv,D_propin_q,Vv_q,'cubic');
                        z_delta = interp2(X,Yv,Z_deltav,D_propin_q,Vv_q,'cubic');
                        z_RPM = interp2(X,Yv,Z_RPMv,D_propin_q,Vv_q,'cubic');
                        z_etha = interp2(X,Yv,Z_ethav,D_propin_q,Vv_q,'cubic');
        ...
        if sensitivity_horizontal == 1
            [X,Yh,Z_Th,Z_Peh,Z_deltah,Z_RPMh,Z_Qh,Z_Eh,Z_ethah,Fig] = Sensitivity_Study_Horizontal_Flight(Aero_TH,SF_prop,RPMMAX_APC,Prop_data,Weight_tier,conv_UNITS,...
            ...
            while query_properties == 1
                ...
                switch answer_prop
                    case 'Yes'
                        ...
                        z_T = interp2(X,Yh,Z_Th,D_propin_q,Vv_q,'cubic');
                        z_Pe = interp2(X,Yh,Z_Peh,D_propin_q,Vv_q,'cubic');
                        z_Q = interp2(X,Yh,Z_Qh,D_propin_q,Vv_q,'cubic');
                        z_delta = interp2(X,Yh,Z_deltah,D_propin_q,Vv_q,'cubic');
                        z_RPM = interp2(X,Yh,Z_RPMh,D_propin_q,Vv_q,'cubic');
                        z_etha = interp2(X,Yh,Z_ethah,D_propin_q,Vv_q,'cubic');
        ...
        if sensitivity_horizontal == 1 && sensitivity_vertical == 1 && diameter_optim== 1
           ...
           [Vvopt_mc,Vhopt_mc,Dopt_mc] = diameteroptimization(X,Yv,Yh,Z_Th,Z_Peh,Z_Eh,Z_ethah,...
                Z_Tv,Z_Pev,Z_Ev,Z_ethav);
...
if STABILITY_STUDY == 1
    ...
    if STABILITY_STUDY_Trim == 1
        only_trim = 0;
        Stability_Pamadi =1;
        [TRIM_RESULTS,Trim_ITER,Stab_Der]  = Calculo_Stability_Derivatives_Abril2020...
    ...
    if STABILITY_STUDY_Trim_varV_m0 == 1
        ...
        for i=1:N_V_VAR
            ...
            for j=1:N_m_VAR
                ...
                [TRIM_RESULTS_calc,Trim_ITER_calc,Stab_Der_calc] = Calculo_Stability_Derivatives_Abril2020...
                    (conditions,Aero,Aero_TH,Geo_tier,Weight_tier,Prop_data,conv_UNITS,...
                    Body_Geo,Design_criteria,Posicion_Palanca,SM_des,XCG_FF,DATA_Ae,Performance,XCG_data,AC_CONFIGURATION,only_trim);
            ...
            if PERFORMANCE_STUDY == 1
                if ANALYSIS_PERFORMANCE_AP_var == 1
                    [Fig] = Generates_Plots_Performance_v1(Geo_tier,Plot_Options,conv_UNITS,Fig,...
                        Segments, handles,Weights_AP_var,fuel_total_var,tiempo_total_var,distancia_total_var,W_var,datos_var,case_AC);
                else
