function modelo = conv_modelo_v4(AC_CONFIGURATION,TRIM_RESULTS,conditions,Trim_ITER,Stab_Der_parts, Geo_tier,...
...
if AC_CONFIGURATION.HTP == 1
    % Arm from Zcg to Zac_HTP
    l_xcg_HTP = Geo_tier.x_xbar_HTP - x_XCG;
    l_zcg_HTP = Geo_tier.z_zbar_HTP - z_XCG;
    z_v = (l_zcg_HTP*cos(trim_alpha) - l_xcg_HTP*sin(trim_alpha));
end
...
if model_conversion == 1
    ...
    deps_dalpha_HTP = Effects.deps_dalpha_HTP;

    if AC_CONFIGURATION.HTP == 1
        modelo.general.downwash = deps_dalpha_HTP;
    ...
    %     b_HTP_s = Geo_tier.b_HTP_s;
    %     AR_HTP_s = Geo_tier.AR_HTP_s;
    %     S_HTP_pv = Geo_tier.S_HTP_pv;
    %     S_HTP_ph = Geo_tier.S_HTP_ph;
    ...
    switch AC_CONFIGURATION.AC_type
        ...
        case 2 % AC_type = 2 - Conventional 2 surface: wing + HTP + VTP
            ...
            if AC_CONFIGURATION.HTP == 1
                if prop_wash_effect == 1
                    eta_HTP_no_afe = afe.eta_HTP_no_afe;
                else
                    % Assumes that the ratio of dynamic pressures is equal
                    % to 1
                    eta_HTP_no_afe = 1;
                ...
                modelo.horizontal.S = Geo_tier.S_HTP;
                modelo.horizontal.eta = eta_HTP_no_afe;
                modelo.horizontal.Cla = Stab_Der_parts.CL_alpha_HTP;
                modelo.horizontal.CLa = Stab_Der_parts.CL_alpha_HTP;
                modelo.horizontal.b = (Geo_tier.b_HTP_s);
                modelo.horizontal.Xca = Geo_tier.x_xbar_HTP;
                modelo.horizontal.Zca = l_zcg_HTP;
                modelo.horizontal.AR = Geo_tier.AR_HTP;
                modelo.horizontal.TR = Geo_tier.lambda_HTP;
                modelo.horizontal.cm_c = 0.25;
                %                 modelo.horizontal.t_c = 0.12; %naca 0012
                modelo.horizontal.LAMc2 = Geo_tier.Lambda_c2_HTP;
                modelo.horizontal.LAMc4 = Geo_tier.Lambda_c4_HTP;
                modelo.horizontal.diedro = Geo_tier.dihedral_HTP;
                modelo.horizontal.xca = Geo_tier.xbar_HTP;
                modelo.horizontal.le_y = (Geo_tier.b_HTP/2)*tan(Geo_tier.Lambda_LE_HTP);
                modelo.horizontal.ct = Geo_tier.cT_HTP;
                modelo.horizontal.MAC = Geo_tier.cmac_HTP;
                modelo.horizontal.MAC_e = Geo_tier.cmac_HTP_e;
                modelo.horizontal.y0_b2 = K_y1_ele_HTP;
                modelo.horizontal.y1_b2 = K_y2_ele_HTP;
        ...
        case 6 % AC_type = 5 - 3 surface: cannard + wing + VTP
            ...
            % K_y2_ele_HTP = Geo_tier.K_y2_ele_HTP;
            %
            % modelo.vertical.S = Geo_tier.S_HTP_pv;
