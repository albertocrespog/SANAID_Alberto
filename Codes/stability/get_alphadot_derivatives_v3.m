function [Stab_Der,Stab_Der_parts] = get_alphadot_derivatives_v3(AC_CONFIGURATION, Stab_Der, Stab_Der_parts,afe,conditions, Performance,Geo_tier,TRIM_RESULTS,Body_Geo,Aero,Effects,...
...
if HTP == 1
    ...
    Vbar_h = ((x_xbar_HTP - x_XCG))/(cmac_w1); %para no mult dos veces *(S_HTP_e/S_ref)
    ...
    % W1 and HTP with no dynamic pressure correction
    CL_alphapunto_HTP = 2*CLalpha_HTP_e*Vbar_h*deps_dalpha_HTP; %eq 4.525 PAMADI
    % Revisar OJO - contribuición de presiópn dinámica en ala HTP
    ...
    if prop_wash_effect == 1
        ...
        CL_alphapunto_HTP_pw = eta_HTP_afe_S_HTP_afe_S_ref*CL_alphapunto_HTP + ...
            eta_HTP_no_afe_S_HTP_no_afe_S_ref*CL_alphapunto_HTP;
    else
        CL_alphapunto_HTP_pw = CL_alphapunto_HTP*conv_HTP;
    end
else
...
%     Stab_Der.CL_alphapunto_HTP_pw = CL_alphapunto_HTP_pw;
...
int_Sb1 = @(x) interp1(x_Area_body(1:end),Area_body(1:end),x).*(x);               %eq 4.514 PAMADI
...
if W1 == 1
    ...
    % W1 and HTP with no dynamic pressure correction %eq 4.539 PAMADI
    % Ojo revisar por la contribución de la presión dinámica
...
if HTP == 1
    CM_alphapunto_HTP = - CL_alphapunto_HTP*Vbar_h;      %eq 4.537 PAMADI
    % W1 and HTP with no dynamic pressure correction
    ...
    if prop_wash_effect == 1
        eta_HTP_afe_S_HTP_afe_S_ref = afe.eta_HTP_afe_S_HTP_afe_S_ref;
        eta_HTP_no_afe_S_HTP_no_afe_S_ref = afe.eta_HTP_no_afe_S_HTP_no_afe_S_ref;
        CM_alphapunto_HTP_pw = eta_HTP_afe_S_HTP_afe_S_ref*CM_alphapunto_HTP + ...
            eta_HTP_no_afe_S_HTP_no_afe_S_ref*CM_alphapunto_HTP;
    else
        CM_alphapunto_HTP_pw = CM_alphapunto_HTP*conv_HTP;
    end
else
...
%     Stab_Der.CM_alphapunto_HTP_pw = CM_alphapunto_HTP_pw;
