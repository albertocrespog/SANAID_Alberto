function F = trim_eqs_v2(x, CL0_ac, CL_alpha_ac, CL_delta, ...
...
F(2) = (CM0_needed + CM_alpha_ac*alpha + CM_delta*delta)*q_inf*S_ref*cmac_w1 + T*(cos(phiT)*cos(psiT)*z_d_T - sin(phiT)*x_d_T);
% F(3) = T - (CD0_ac + CD_alpha_ac*alpha + CD_delta*delta)/cos(alpha);
F(3) = m_TOW*g*sin(gamma) + (CD0_ac + CD_alpha_ac*alpha + CD_delta*delta)*q_inf*S_ref - T*(cos(phiT)*cos(psiT)*cos(alpha) + sin(phiT)*sin(alpha));
