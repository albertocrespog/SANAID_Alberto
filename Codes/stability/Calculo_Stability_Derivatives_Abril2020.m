function [TRIM_RESULTS,Trim_ITER,Stab_Der,Stab_Der_parts] = ...
    Calculo_Stability_Derivatives_Abril2020(conditions,Aero,Aero_TH,Geo_tier,Weight_tier,Prop_data,conv_UNITS,...
...
%     %%%%%%%%%%%%%%%%%%%%%%CÁLCULO DEL UPWASH%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     
%     deps_dalpha_clean = upwash_calc(AR_w1, X_w, X_c, c_m);
%     deps_dalpha_poff = 0; % Assumes not affecting
...
[Stab_Der_parts, afe] = propwash_influence(AC_CONFIGURATION,Geo_tier,Posicion_Palanca,prop_wash_effect,conditions,Propulsion,Performance,Aero,Effects);
...
% [X_NP]=calc_ca(AC_type,conditions,Performance,Geo_tier,Body_Geo,Stab_Der_parts, Effects,Design_criteria,AC_CONFIGURATION)%% To replace by wind tunnel models
...
% [Stab_Der_parts,Trim_ITER,Stab_Der,TRIM_RESULTS] = get_CMalpha_SM(AC_CONFIGURATION,Geo_tier,Body_Geo,Stab_Der,Stab_Der_parts,conv_UNITS,Effects,OUTPUT_read_XLSX,SM_des,X_NP,conditions)
% %% Propulsive Model
% %%%%%%%%%%%%%%%%%%%%%%%%%%%DERIVADA PROPULSIVA LONGITUDINAL%%%%%%%%%%%%%%%%
% CD_Total_prel = C_D0 + C_D1*CL + C_D2*CL^2;  %polar aeronave
% Drag = q_inf*S_ref*CD_Total_prel;
...
%     CM_0_can_wrt_XCG = CM_0_can_ae + CM0_can_CL0_can;
...
if only_trim == 0
 ...
 [Stab_Der, Stab_Der_parts] = get_pitch_derivatives(AC_CONFIGURATION, Stab_Der, Stab_Der_parts,afe,conditions, Performance,Geo_tier,TRIM_RESULTS,Body_Geo,Aero)
...
%     CLu = (Mach/(1 - Mach^2))*CL;                   %para vuelo subsónico
%     CZu = -2*CL - CLu ;
...
%     Stab_Der.CXq = CXq;
%     Stab_Der.CZq = CZq;
Stab_Der.CM_alpha_ac = Stab_Der.CM_alpha_ac_des;
TRIM_RESULTS.x_XCG = TRIM_RESULTS.x_XCG_des;
TRIM_RESULTS.CM_alpha_ac = TRIM_RESULTS.CM_alpha_ac_des;
TRIM_RESULTS.SM = TRIM_RESULTS.SM_des;
...
[Stab_Der_parts,Stab_Der] = get_beta_derivatives(AC_CONFIGURATION,modelo,Stab_Der,Stab_Der_parts,Geo_tier,TRIM_RESULTS,Body_Geo)
%     %% Cyb
%     % ASPro Cyb
...
%     Cndeltaa = Stab_Der.Cndeltaa;
    ...
    if d_rudder ==1
        alpha = 0; %preguntar
        Stab_Der = get_deltar_latdir_deriv(modelo,Stab_Der,Stab_Der_parts,afe,alpha);
...
%         Stab_Der = getCndr(modelo,trim_alpha,Stab_Der);
    ...
    if d_rudvtr == 1
        alpha = 0; %preguntar
        Stab_Der = get_delta_rv_latdir_deriv(modelo,Stab_Der,Stab_Der_parts,afe,alpha);
