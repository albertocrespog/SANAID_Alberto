    function [Stab_Der, Stab_Der_parts] = get_pitch_derivatives(AC_CONFIGURATION, Stab_Der, Stab_Der_parts,afe,conditions, Performance,Geo_tier,Body_Geo,Aero,OUTPUT_read_XLSX)
...
if HTP == 1
    ...
    Vbar_h = ((x_xbar_w2 - x_XCG)/cmac_w1); %Sin *(S_w2_e/S_ref) puesto que luego
    %multiplicas por eta_w2_no_afe_S_w2_no_afe_Sref y ya va
    %multiplicado ahí
...
if Vee == 1
    ...
    Vbar_vee = ((x_xbar_w2 - x_XCG)/cmac_w1); %Sin *(S_w2_e/S_ref) puesto que luego
    %multiplicas por eta_w2_no_afe_S_w2_no_afe_Sref y ya va
    %multiplicado ahí
...
%     Vbar_can = ((x_xbar_can + x_XCG)/cmac_w1)*(S_can_s/S_ref);
    Vbar_can = ((x_xbar_can + x_XCG)/cmac_w1);%Sin *(S_w2_e/S_ref) puesto que luego
...
CLq = CLq_WB + CLq_HTP_pw + CLq_Vee_pw + CLq_can_pw + CLq_nac_pw; %Pamadi
...
if W1 == 1
    ...
    CLalpha_w1_2D = clalpha_w2_M0/sqrt(1-Mach^2); %Pamadi 3.17
