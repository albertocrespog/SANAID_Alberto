function Storing_STABILITY_DATA_2C = Conduct_Stability_Study2B_v2(conditions,Prop_data,Propulsion,conv_UNITS,Posicion_Palanca,...
...
gamma_min = OUTPUT_read_XLSX.Stability_flags.gamma_min;
gamma_max = OUTPUT_read_XLSX.Stability_flags.gamma_max;
N_gamma_var = OUTPUT_read_XLSX.Stability_flags.N_gamma_var;
gamma_VAR = linspace(gamma_min,gamma_max,N_gamma_var);
Plot_Options.gamma_VAR = gamma_VAR;
...
for i=1:N_gamma_var
    ...
    for j=1:N_V_VAR
        ...
        % Storing
        Restrictions_var_V_gamma{i,j} = Restrictions;
        TRIM_RESULTS_var_V_gamma{i,j} = TRIM_RESULTS_calc_var_V_m_v;
        Trim_ITER_var_V_gamma{i,j} = Trim_ITER_calc_var_V_m_v;
        Stab_Der_var_V_gamma{i,j} = Stab_Der_calc_var_V_m_v;
        Stab_Der_parts_V_gamma{i,j} = Stab_Der_parts_calc_var_V_m_v;
        Propulsion_var_V_gamma{i,j} =  Propulsion_calc_var_V_m_v;
        ...
        if OUTPUT_read_XLSX.STUDY_flags.STABILITY_STUDY_Long_dyn == 1
            ...
            Stab_Dyn_Long_var_V_gamma{i,j} = longitudinal_analysis(Performance,Stab_Der_calc_var_V_m_v,conv_UNITS,StabilityModel,...
                Weight_tier,TRIM_RESULTS_calc_var_V_m_v,Geo_tier,Variable_Study,conditions,OUTPUT_read_XLSX,show_messages_screen,filenameS);
        else
            Dummy = 0;
            Stab_Dyn_Long_var_V_gamma = Dummy;
        end
        %ESTABILIDAD DINAMICA LATERAL-DIRECCIONAL%%%%%%%%%%%%%%%%%%%%%%
        if OUTPUT_read_XLSX.STUDY_flags.STABILITY_STUDY_LatDir_dyn == 1
            show_messages_screen = 0;
            Stab_Dyn_LatDir_var_V_gamma{i,j} = lateral_directional_analysis(Performance,Stab_Der_calc_var_V_m_v,conv_UNITS,StabilityModel,...
                Weight_tier,TRIM_RESULTS_calc_var_V_m_v,Geo_tier,Variable_Study,conditions,OUTPUT_read_XLSX,show_messages_screen,filenameS);
        else
            Dummy = 0;
            Stab_Dyn_LatDir_var_V_gamma = Dummy;
        end
    end
end
...
if OUTPUT_read_XLSX.MATLAB_flags.save_MAT == 1
    Storing_STABILITY_DATA_2C = Saving_data_Stability_varV_vargamma(TRIM_RESULTS_var_V_gamma,Trim_ITER_var_V_gamma,Stab_Der_var_V_gamma,Stab_Der_parts_V_gamma,...
