function [Stab_Der_parts, afe] = propwash_influence_v3(AC_CONFIGURATION,Geo_tier,Posicion_Palanca,conditions,Propulsion,Performance,Aero,Effects,OUTPUT_read_XLSX,Body_Geo)
...
if prop_wash_effect == 1
    ...
    if Can == 1
        ...
        Stab_Der_parts.CL0_can_e_corrected = 0;
        Stab_Der_parts.CLalpha_can_e_pw = 0;
    end

    if HTP == 1

        S_HTP_pw = Geo_tier.S_HTP_pw;
        ...
        eta_HTP = Effects.eta_HTP;

        CL0_HTP      = Aero.CL_0_HTP_CR;
        CL_alpha_HTP = Aero.CL_alpha_HTP_CR;


        if wingspan2bodydiam_HTP <= 2 || flagwingspan2bodydiam==1
            CL0_HTP_e     = CL0_HTP*S_HTP_e/Geo_tier.S_HTP;
        ...
        S_HTP_afe = S_HTP_pw;
        S_HTP_no_afe = S_HTP_s - S_HTP_afe;
        q_HTP_no_afe    = eta_HTP*q_inf;
        ...
        if Posicion_Palanca == 0
            ...
            V_HTP = sqrt(eta_HTP)*V + 2*v_i;
            q_HTP_afe       = 0.5*rho*V_HTP^2;
        end
        ...
        %     end
        %
        eta_HTP_afe = (q_HTP_afe/q_inf);
        eta_HTP_no_afe = (q_HTP_no_afe/q_inf);
        afe.eta_HTP_afe = eta_HTP_afe;
        afe.eta_HTP_no_afe = eta_HTP_no_afe;
        eta_HTP_afe_S_HTP_afe_S_ref = eta_HTP_afe*(S_HTP_afe/S_ref);
        eta_HTP_no_afe_S_HTP_no_afe_S_ref = eta_HTP_no_afe*(S_HTP_no_afe/S_ref);
        afe.eta_HTP_afe_S_HTP_afe_S_ref = eta_HTP_afe_S_HTP_afe_S_ref;
        afe.eta_HTP_no_afe_S_HTP_no_afe_S_ref = eta_HTP_no_afe_S_HTP_no_afe_S_ref;
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%CLalfa%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %% Correction of Lift curve slope of HTP associated to prop wash
        % eta_w1_afe = (q_w1_afe/q_inf)*(S_w1_afe/S_ref)
        % eta_w1_no_afe = (q_w1_no_afe/q_inf)*(S_w1_no_afe/S_ref)
        CLalpha_HTP_e_pw = eta_HTP_afe_S_HTP_afe_S_ref*CLalpha_HTP_e + eta_HTP_no_afe_S_HTP_no_afe_S_ref*CLalpha_HTP_e;
        CL0_HTP_e_corrected = eta_HTP_afe_S_HTP_afe_S_ref*CL0_HTP_e + eta_HTP_no_afe_S_HTP_no_afe_S_ref*CL0_HTP_e;
        Stab_Der_parts.CL0_HTP_e_corrected = CL0_HTP_e_corrected;
        Stab_Der_parts.CLalpha_HTP_e_pw = CLalpha_HTP_e_pw;

    else
        Stab_Der_parts.CL0_HTP_e_corrected = 0;
        Stab_Der_parts.CLalpha_HTP_e_pw = 0;
    end
    ...
    if Vee == 1
        ...
        Stab_Der_parts.CL0_vee_e_corrected = 0;
        Stab_Der_parts.CLalpha_vee_e_pw = 0;
    end
    ...
    if Vee2 == 1
        ...
        Stab_Der_parts.CL0_vee2_e_corrected = 0;
        Stab_Der_parts.CLalpha_vee2_e_pw = 0;
    end
...
%     end
        ...
        %             conv_VTP = S_HTP/S_w1; %adimensionalizados con la S_HTP_total en flow5, no con la expuesta;
        %         end
        %     case 3
        %         % If there is HTP the Aerodynamic studies in FLOW have been conducted
        %         % HTP + VTP hence the area used for adimentionalizing is the one from the HTP
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%CLalfa%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %% Correction of Lift curve slope of VTP associated to prop wash
        CLalpha_VTP_e_pw = eta_VTP_afe_S_VTP_afe_S_ref*CLalpha_VTP + eta_VTP_no_afe_S_VTP_no_afe_S_ref*CLalpha_VTP;
        CL0_VTP_e_corrected = eta_VTP_afe_S_VTP_afe_S_ref*CL0_VTP_e + eta_VTP_no_afe_S_VTP_no_afe_S_ref*CY_0_VTP_CR;
        Stab_Der_parts.CL0_VTP_e_corrected = CL0_VTP_e_corrected;
        Stab_Der_parts.CLalpha_VTP_e_pw = CLalpha_VTP_e_pw;
    else
        Stab_Der_parts.CL0_VTP_e_corrected = 0;
        Stab_Der_parts.CLalpha_VTP_e_pw = 0;
    end

    if Nac == 1
        Stab_Der_parts.CL0_Nac_e_corrected = 0;
        Stab_Der_parts.CLalpha_Nac_e_pw = 0;
    else
        Stab_Der_parts.CL0_Nac_e_corrected = 0;
        Stab_Der_parts.CLalpha_Nac_e_pw = 0;
    end
    ...
    if W1 == 1
        ...
        q_w1_no_afe = q_inf;
        eta_w1_no_afe = (q_w1_no_afe/q_inf);
        eta_w1_no_afe_S_w1_no_afe_S_ref = eta_w1_no_afe*(S_w1_e/S_ref);
        CL0_w1_e_corrected = eta_w1_no_afe_S_w1_no_afe_S_ref*CL0_w1_e;
        CLalpha_w1_e_pw    = eta_w1_no_afe_S_w1_no_afe_S_ref*CLalpha_w1_e;
        Stab_Der_parts.CL0_w1_e_corrected = CL0_w1_e_corrected;
        Stab_Der_parts.CLalpha_w1_e_pw = CLalpha_w1_e_pw;
    end
    ...
    if Can == 1
        ...
        % conv_can = S_can_s/S_ref;%adimensionalizados con la S_can total en flow5, no con la expuesta;
        ...
        q_can_no_afe = q_inf;
        eta_can_no_afe = (q_can_no_afe/q_inf);
        eta_can_no_afe_S_can_no_afe_S_ref = eta_can_no_afe*(S_can_e/S_ref);
        CL0_can_e_corrected = eta_can_no_afe_S_can_no_afe_S_ref*CL0_can_e;
        CLalpha_can_e_pw    = eta_can_no_afe_S_can_no_afe_S_ref*CLalpha_can_e;
        Stab_Der_parts.CL0_can_e_corrected = CL0_can_e_corrected;
        Stab_Der_parts.CLalpha_can_e_pw = CLalpha_can_e_pw;
    ...
    if HTP == 1
        ...
        % CLalpha_HTP_e_pw = CLalpha_HTP_e*conv_HTP;
        % CL0_HTP_e_corrected = CL0_HTP_e*conv_HTP;
        % Stab_Der_parts.CL0_HTP_e_corrected = CL0_HTP_e_corrected;
        % Stab_Der_parts.CLalpha_HTP_e_pw = CLalpha_HTP_e_pw;

        q_HTP_no_afe = q_inf;
        eta_HTP_no_afe = (q_HTP_no_afe/q_inf);
        eta_HTP_no_afe_S_HTP_no_afe_S_ref = eta_HTP_no_afe*(S_HTP_e/S_ref);
        CL0_HTP_e_corrected = eta_HTP_no_afe_S_HTP_no_afe_S_ref*CL0_HTP_e;
        CLalpha_HTP_e_pw    = eta_HTP_no_afe_S_HTP_no_afe_S_ref*CLalpha_HTP_e;
        Stab_Der_parts.CL0_HTP_e_corrected = CL0_HTP_e_corrected;
        Stab_Der_parts.CLalpha_HTP_e_pw = CLalpha_HTP_e_pw;
    ...
    if Vee == 1
        ...
        q_vee_no_afe = q_inf;
        eta_vee_no_afe = (q_vee_no_afe/q_inf);
        eta_vee_no_afe_S_vee_no_afe_S_ref = eta_vee_no_afe*(S_vee_e/S_ref);
        CL0_vee_e_corrected = eta_vee_no_afe_S_vee_no_afe_S_ref*CL0_vee_e;
        CLalpha_vee_e_pw    = eta_vee_no_afe_S_vee_no_afe_S_ref*CLalpha_vee_e;
        Stab_Der_parts.CL0_vee_e_corrected = CL0_vee_e_corrected;
        Stab_Der_parts.CLalpha_vee_e_pw = CLalpha_vee_e_pw;
    ...
    if Vee2 == 1
        ...
        q_vee2_no_afe = q_inf;
        eta_vee2_no_afe = (q_vee2_no_afe/q_inf);
        eta_vee2_no_afe_S_vee2_no_afe_S_ref = eta_vee2_no_afe*(S_vee2_e/S_ref);
        CL0_vee2_e_corrected = eta_vee2_no_afe_S_vee2_no_afe_S_ref*CL0_vee2_e;
        CLalpha_vee2_e_pw    = eta_vee2_no_afe_S_vee2_no_afe_S_ref*CLalpha_vee2_e;
        Stab_Der_parts.CL0_vee2_e_corrected = CL0_vee2_e_corrected;
        Stab_Der_parts.CLalpha_vee2_e_pw = CLalpha_vee2_e_pw;
    else
        Stab_Der_parts.CL0_vee2_e_corrected = 0;
    ...
    if VTP == 1
        ...
        q_VTP_no_afe = q_inf;
        eta_VTP_no_afe = (q_VTP_no_afe/q_inf);
        eta_VTP_no_afe_S_VTP_no_afe_S_ref = eta_VTP_no_afe*(S_VTP_e/S_ref);
        CL0_VTP_e_corrected = eta_VTP_no_afe_S_VTP_no_afe_S_ref*CY_0_VTP_CR;
        CLalpha_VTP_e_pw    = eta_VTP_no_afe_S_VTP_no_afe_S_ref*CLalpha_VTP_e;
        Stab_Der_parts.CL0_VTP_e_corrected = CL0_VTP_e_corrected;
        Stab_Der_parts.CLalpha_VTP_e_pw = CLalpha_VTP_e_pw;
