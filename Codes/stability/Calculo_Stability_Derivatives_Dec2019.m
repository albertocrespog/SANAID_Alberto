function [TRIM_RESULTS,Trim_ITER,Stab_Der] = ...
    ...
    Body_Geo,Design_criteria,Posicion_Palanca,SM_des,XCG_FF,DATA_Ae,Performance,XCG_data,AC_CONFIGURATION)
...
S_w3 = Geo_tier.S_w3; 
S_w3_e = Geo_tier.S_w3_e;
S_w3_s = Geo_tier.S_w3_s; 
% prop wash area affecting surfaces
S_w3_pw = Geo_tier.S_w3_pw;
S_w3_afe = S_w3_pw;
...
% V_t = ((x_xbar_w2 - x_XCG)/cmac_w1)*(S_w2_e/S_ref);
...
% CLalpha_w3 = Aero.CL_alpha_w3_CR;
% CLalpha_w3_e = CLalpha_w3;

% CL0_wb = Aero3D.CL0_wb_CR;
CL0_w1 = Aero.CL_0_w1_CR;
CL0_w2 = Aero.CL_0_w2_CR;
% CL0_w3 = Aero.CL_0_w3_CR;
CL0_w1_e = CL0_w1;
CL0_w2_e = CL0_w2;
% CL0_w3_e = CL0_w3;
...
eta_w3 = eta_w3_power_off + eta_w3_power;
...
if Posicion_Palanca == 0
    ...
    S_w3_no_afe = S_w2_s - S_w3_afe;
    V_w1 = V;
    V_w2 = V;
    V_w3 = V;
    q_w1_afe    = q_inf;
    q_w2_afe    = eta_w2*q_inf;                    %presion dinamica afectada
    q_w3_afe    = eta_w3*q_inf;                    %presion dinamica afectada
    q_w1_no_afe = q_inf;
    q_w2_no_afe = eta_w2*q_inf;                    %presion dinamica no afectada
    q_w3_no_afe = eta_w3*q_inf;                    %presion dinamica no afectada
else
    S_w1_afe = S_w1_pw;
    S_w2_afe = S_w2_pw;
    S_w3_afe = S_w3_pw;
    S_w1_no_afe = S_w1 - S_w1_afe;
    S_w2_no_afe = S_w2_s  - S_w2_afe;
    S_w3_no_afe = S_w3_s  - S_w3_afe;
    V_w1 = V + v_i;
    V_w2 = sqrt(eta_w2)*V + 2*v_i;
    V_w3 = sqrt(eta_w3)*V + 2*v_i;
    q_w1_afe       = 0.5*rho*V_w1^2;
    q_w2_afe       = 0.5*rho*V_w2^2;                 %presion dinamica afectada
    q_w3_afe       = 0.5*rho*V_w3^2;                 %presion dinamica afectada
    q_w1_no_afe   = q_inf;
    q_w2_no_afe    = eta_w2*q_inf;                    %presion dinamica no afectada
    q_w3_no_afe    = eta_w3*q_inf;                    %presion dinamica no afectada
end
...
eta_w3_afe_S_w3_afe_S_ref = eta_w3_afe*(S_w3_afe/S_ref);
eta_w3_no_afe_S_w3_no_afe_S_ref = eta_w3_no_afe*(S_w3_no_afe/S_ref);
...
% CLalpha_w3_e_pw = eta_w3_afe_S_w3_afe_S_ref*CLalpha_w3_e + eta_w3_no_afe_S_w3_no_afe_S_ref*CLalpha_w3_e;
...
CLalpha_w2_corrected = CLalpha_wb_w2*1.3;
...
CL_delta_e          = alpha_delta_e*CLalpha_w2_corrected*(S_w2_e/S_ref);
...
CL_i_vee = (eta_w2_afe_S_w2_afe_S_ref*CLalpha_w2_corrected + eta_w2_no_afe_S_w2_no_afe_S_ref*CLalpha_w2_corrected);
...
% The change in airplane angle-of-attack due to elevator deflection at zero deflection
% The airplane drag-coefficient-due-to-stabilizer-incidence derivative:
CD_i_vee          = 2*CL0_w1w2b*C_D2*(eta_w2_afe_S_w2_afe_S_ref*CLalpha_w2_corrected + eta_w2_no_afe_S_w2_no_afe_S_ref*CLalpha_w2_corrected);
...
Trim_ITER.CL_delta_e= CL_delta_rv;
Trim_ITER.CD_delta_e= CD_delta_rv;
Trim_ITER.CM_delta_e= CM_delta_rv;
...
den_delta_e = -CL_alpha_w1w2b*CM_delta/CM_alpha_w1w2b + CL_delta;
...
CL = CL0_w1w2b + CL_alpha_w1w2b*trim_alpha + CL_delta*trim_delta_e;

% CL_w1
CL_w1_b = CL0_w1_e_corrected + CLalpha_w1_corrected*i_w1 + CLalpha_wb_w1*trim_alpha;
% CL_w2
CL_w2 = CL0_w2_e_corrected + CLalpha_w2_corrected*(i_w2 - eps_w2) + CLalpha_w2_e_pw*(downwash)*trim_alpha + CL_delta*trim_delta_e;
% T1otal CL
...
CM = CM0_w1w2b + CM_alpha_w1w2b*trim_alpha + CM_delta*trim_delta_e;
TRIM_RESULTS.CM = CM;

%% Most Forward XCG
x_XCG_fwd =  -(delta_rudvtr_min + (CM0_w1w2b/CM_delta))/(alpha_max_w1_ope*D2R*CL_alpha_w1w2b/(CM_delta*cmac_w1)) + X_NP;
%% Most Forward XCG
SM_min = 0.10;
...
% CLq_wb_w1 = CLq_w1 + CLq_fus; %eq 4.488 PAMADI
...
CLq = CLq_w1_pw + CLq_fus + CLq_w2_pw;

CZq = -CLq;
% Storing DATA
Stab_Der.CLq_w1 = CLq_w1_pw;
...
CL_alphapunto_w1_pw = eta_w1_afe_S_w1_afe_S_ref*CL_alphapunto_wb_w1_part1 +...
...
CL_alphapunto_fus = CL_alphapunto_wb_w1_part2;                   %eq 4.527 PAMADI
...
CLalphapunto = CL_alphapunto_w1_pw + CL_alphapunto_fus + CL_alphapunto_w2_pw;
% Storing DATA
Stab_Der.CL_alphapunto_w1 = CL_alphapunto_w1_pw;
Stab_Der.CL_alphapunto_w2 = CL_alphapunto_w2_pw;
...
CM_alphapunto_w1_pw = eta_w1_afe_S_w1_afe_S_ref*CM_alphapunto_wb_w1_part1 +...
...
% W1 and w2 with no dynamic pressure correction
%% Revisar OJO - contribuición de presiópn dinámica en ala w1
% CM_alphapunto_wb_w1_pw = CM_alphapunto_wb_w1 + CM_alphapunto_fus;
...
CMalphapunto = CM_alphapunto_w1_pw + CM_alphapunto_fus + CM_alphapunto_w2_pw; %eq 4.538 PAMADI
% Storing DATA
Stab_Der.CM_alphapunto_w1 = CM_alphapunto_w1_pw;
Stab_Der.CM_alphapunto_w2 = CM_alphapunto_w2_pw;
...
switch case_AC
    ...
    case 2 % AC_type = 2 - Conventional 2 surface: wing + HTP + VTP
        ...
        % VTP
        modelo.vertical.S = Geo_tier.S_w3_pv;
        modelo.vertical.eta = eta_w3_no_afe;
        modelo.vertical.Cla = CLalpha_wb_w3*(sin(dihedral_w2))^2;
        modelo.vertical.Cla = CLalpha_wb_w3;
        modelo.vertical.CLa = CLalpha_wb_w3*(sin(dihedral_w2))^2;
        modelo.vertical.CLa = CLalpha_wb_w3;
        modelo.vertical.b = (b_w3_s)*sin(dihedral_w3);
        modelo.vertical.Xca = x_xbar_w3;
        modelo.vertical.Zca = l_zcg_w3;
        modelo.vertical.AR = (((b_w3_s/2)*sin(dihedral_w3))^2)/(S_w3_pv);
        modelo.vertical.TR = lambda_w3;
        modelo.vertical.cm_c = 0.25*cmac_w3;
        modelo.vertical.t_c = 0.12; %naca 0012
        modelo.vertical.LAMc2 = Lambda_c2_w3;
        modelo.vertical.LAMc4 = Lambda_c4_w3;
        modelo.vertical.diedro = dihedral_w3;
        modelo.vertical.xca = xbar_w3;
        modelo.vertical.le_y = (b_w3)*tan(Lambda_LE_w3);
        modelo.vertical.ct = cT_w3;
        modelo.vertical.MAC = cmac_w3;
        modelo.vertical.MAC_e = cmac_w3_e;
