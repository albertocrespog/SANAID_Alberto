function [Stab_Der_parts,  Trim_ITER] = getCLalpha_v2(AC_CONFIGURATION,Body_Geo,Geo_tier,body_interference,Stab_Der_parts, Effects,Design_criteria, OUTPUT_read_XLSX)
...
if Vee==1
    ...
    a_WB_vee = (KN_vee + KWB_vee + KBW_w1)*CLalpha_Vee_e_pw*(S_vee_s/S_vee); %eq 3.24 PAMADI
...
if Vee2==1
    ...
    a_WB_vee2 = (KN_vee2 + KWB_vee2 + KBW_w1)*CLalpha_vee2_e_pw*(S_vee2_s/S_vee2); %eq 3.24 PAMADI
...
elseif AC_type == 8
    if wingspan2bodydiam <= 2 || flagwingspan2bodydiam==1
        CL_alpha_ac = CL_alpha_wb_w1 + CL_alpha_Vee*(downwash) + CL_alpha_Vee2*(downwash);
    else
        CL_alpha_ac = CL_alpha_w1 + CLa_fus + CL_alpha_Vee*(downwash) + CL_alpha_Vee2*(downwash);
    end
    
    %     X_NP = (CL_alpha_w1*XAC_WB + CL_alpha_Vee*(downwash)*x_xbar_w2)/CL_alpha_ac;
    CL0_ac = CL0_w1_e_corrected + CL_alpha_w1*i_w1 + CL0_Vee_e_corrected + CL_alpha_Vee*(i_w2 - eps_vee) + CL0_Vee2_e_corrected + CL_alpha_Vee2*(i_vee2 - eps_vee2);
    Trim_ITER.CL0_w1_e_corrected = CL0_w1_e_corrected;
    Trim_ITER.CL0_Vee_e_corrected = CL0_Vee_e_corrected;
    Trim_ITER.CL0_ac = CL0_ac;
    % Store values
