function [TRIM_RESULTS,Trim_ITER,Stab_Der] = ...
    Calculo_Stability_Derivatives_June2019(conditions,Aero,Aero_TH,Geo_tier,Weight_tier,Prop_data,conv_UNITS,...
...
SR_SV = (0.435*0.089)/(Geo_tier.S_w2_pv/2);
tau_r_S = 0.44;% 25% cuerda 
% tau_r = 0.6; % 40% cuerda 
% tau_r = 0.5; % 30% cuerda 
% tau_r = 0.65;
% Cydeltar = tau_r_S*CLalpha_w2_corrected*((sin(dihedral_w2))^2)*(Geo_tier.S_w2_pv/S_ref)
% 
Cydeltarv = -eta_vee*Cy_vee_beta*(S_vee/S_ref)*bet_delta_rv*tan(dihedral_w2)
CLalpha_w2_corrected*((sin(dihedral_w2))^2)
Stab_Der = getCydeltarv(modelo,trim_alpha,Stab_Der);
% 
% pause
