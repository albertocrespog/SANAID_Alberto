function Storing_PERFORMANCE_DATA_21 = Conduct_Performance_Optimization_Perfo1_v1(AC_CONFIGURATION,case_AC,Propulsion,Prop_data,conv_UNITS,...
...
V_VAR = OUTPUT_read_XLSX.IPP_flags.Variable_Study1.V_VAR;

h_inicial_cr = OUTPUT_read_XLSX.IPP_flags.h_inicial_cr;
...
V_stall_min_low = sqrt(2*m_VAR_min*conv_UNITS.g/(Geo_tier.S_w1*rho_init*CL_max_ac));
V_stall_min_high = sqrt(2*m_VAR_max*conv_UNITS.g/(Geo_tier.S_w1*rho_init*CL_max_ac));
V_min_ope_low = 1.2*V_stall_min_low;
V_min_ope_high = 1.2*V_stall_min_high;

% Variation of speeds
if V_low < V_stall_min_low;
...
for i=1:length(m_bat_VAR)
    ...
    V_stall(i) = sqrt(2*m_VAR(i)*conv_UNITS.g/(Geo_tier.S_w1*rho_init*CL_max_ac));
    ...
    for j=1:length(V_VAR)
        ...
        % Calculations
        Storing_PERFORMANCE_21{i,j} = Conduct_Performance_Optimization_Perfo0_v1(AC_CONFIGURATION,case_AC,Propulsion,Prop_data,conv_UNITS,...
            OUTPUT_read_XLSX,Storing_AERO_DATA,Storing_GEO_DATA,Storing_WEIGHT_DATA,Storing_PROPULSION_DATA,filenameS);
...
if OUTPUT_read_XLSX.MATLAB_flags.save_MAT == 1
    Storing_PERFORMANCE_DATA_21 = Saving_data_Performance_21(Storing_PERFORMANCE_21,Plot_Options,Restrictions_var_V_m,OUTPUT_read_XLSX,Storing_PROPULSION_DATA,filenameS);
