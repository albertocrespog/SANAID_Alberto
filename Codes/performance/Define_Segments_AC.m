function Segments = Define_Segments_AC(Aero,Aero_TH,case_AC,conv_UNITS)
...
switch case_AC
    case 1 % case_AC = 1 - EMERGENTIA 1:1
        %% User needs to define the inputs for eachj segment
        cruise_mode = 2;
        climb_mode = 2;
        turn_mode = 5;
        descent_mode = 2; 
    ...
    case 5 % WIG
        ...
        % Climb
        h_inicial_cl = 0; % 1: ALTURA INICIAL - [m]
        h_final_cl = 10; % 1: ALTURA FINAL - [m]
        gamma_cl = 3*D2R; % 2: GAMMA DE SUBIDA  - [-]
        Mach_cl = 0.14; % 3: MACH DE VUELO  - [-]
        TAS_cl = 125; % 4: VELOCIDAD TAS  - [m/s]
        EAS_cl = 125; % 5: VELOCIDAD EAS  - [m/s]
        delta_T_cl = 0.95; % 6: PALANCA DE GASES  - [-]
        ...
        % Cruise
        h_inicial_cr = 10;% - [m] % Altura inicial
        ...
        V_ini_cr = 120;% - [m/s] % 5: VELOCIDAD INICIAL 
        V_fin_cr = 125;% - [m/s] % 6: VELOCIDAD FINAL
        fuel_cr = 260000;% - [kg] % 7: COMBUSTIBLE A QUEMAR 
        Cd0_cr = -1;% - [] % 8: CDO = F(M)  CD0: CD = CD0 + K1*CL^2 - K2*CL
        k1_cr = -1;% - []% 9: K1 = F(M)  K1: CD = CD0 + K1*CL^2 - K2*CL
        k2_cr = -1;% - [] % 10: K2 = F(M) K2: CD = CD0 + K1*CL^2 - K2*CL
