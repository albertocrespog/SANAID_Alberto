function Storing_PERFORMANCE_DATA_26 = Conduct_Performance_Optimization_Perfo6_v1(AC_CONFIGURATION,case_AC,Propulsion,Prop_data,conv_UNITS,...
...
for i=1:length(m_bat_VAR)
    ...
    for j=1:length(V_VAR)
        ...
        % Calculations
        Storing_PERFORMANCE_26{i,j} = Conduct_Performance_Optimization_Perfo0_v1(AC_CONFIGURATION,case_AC,Propulsion,Prop_data,conv_UNITS,...
            OUTPUT_read_XLSX,Storing_AERO_DATA,Storing_GEO_DATA,Storing_WEIGHT_DATA,Storing_PROPULSION_DATA,filenameS);
...
if OUTPUT_read_XLSX.MATLAB_flags.save_MAT == 1
    Storing_PERFORMANCE_DATA_26 = Saving_data_Performance_varV_M(Storing_PERFORMANCE_26,Plot_Options,Restrictions_var_V_m,OUTPUT_read_XLSX,Storing_PROPULSION_DATA,filenameS);
