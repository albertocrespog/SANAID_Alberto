function [fuel_subida,tiempo_subida,S_subida,datos] = analisis_subida_v1(propul,aerodinamica,subida,W_inicial,h_inicial,opcion,i,datos)
...
datos(i).subida.tiempo = (H-H(1))./V_v';
datos(i).subida.fuel = (Newt(1) - Newt)./g; %fuel consumido acumulado
datos(i).subida.distancia = (H-H(1))./tan(gamma);
datos(i).subida.palanca = palanca;
%datos(i).subida.empuje = empuje;
datos(i).subida.empuje = CL2;
datos(i).subida.veloc_vertical = V_v;
datos(i).subida.L_D = L_D;
datos(i).subida.peso = Newt;
% datos(i).subida.techo = techo;
datos(i).subida.gamma = gamma; %%% ESCALAR %%%
datos(i).subida.velocidad = velocidad;
datos(i).subida.altura = H;
...
datos(i).subida.tiempo = cumsum(tiempo);
datos(i).subida.fuel = cumsum(fuel); %fuel consumido acumulado
datos(i).subida.distancia = cumsum(distancia);
datos(i).subida.palanca = delta_T;%%% ESCALAR %%%
%datos(i).subida.empuje = empuje;
datos(i).subida.empuje = CL2;
datos(i).subida.veloc_vertical = V_v;
datos(i).subida.L_D = L_D; %%% ESCALAR %%%
datos(i).subida.peso = w;
datos(i).subida.peso(end) = '';
datos(i).subida.gamma = gamma;
datos(i).subida.gamma(end) = '';
%datos(i).subida.techo = techo;
datos(i).subida.velocidad = velocidad;
datos(i).subida.altura = H(1:end-1);
...
datos(i).subida.tiempo = cumsum(tiempo);
datos(i).subida.fuel = cumsum(fuel);
datos(i).subida.distancia = cumsum(distancia);
datos(i).subida.palanca = delta_T;
datos(i).subida.empuje = empuje;
datos(i).subida.veloc_vertical = V_v;
datos(i).subida.L_D = L_D;
datos(i).subida.peso = w(1:end-1);
%datos(i).subida.techo = techo;
datos(i).subida.gamma = gam;
datos(i).subida.velocidad = velocidad;
datos(i).subida.altura = H(1:end-1);
...
global flag_palanca 
if max(datos(i).subida.palanca) > 1.2,
    flag_palanca = 1;
else
    flag_palanca = 0;
end
