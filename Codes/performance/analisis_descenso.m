function [fuel_descenso,mbaterias_descenso,energia_descenso,tiempo_descenso,S_descenso,datos] = analisis_descenso(propul,aerodinamica,descenso,W_inicial,h_inicial,opcion,datos,i,Prop_data,data_electric)
...
switch opcion
    case 1
        ...
        if tipo_motor==4
            % dEdh= P_sol./(Vvect'*sin(gamma).*eta_mp(nps_sol));
            dEdh= P_sol./(Vvect'*sin(gamma).*eta_mp(nps_sol)*eta_m); %dEdh=dEdx*dxdh=dEdt*dxdt*1/tan(gamma)=P/(V*tan(gamma))
        ...
        if tipo_motor==4
            ...
            fuel_descenso = 0;
            energia_descenso=ENERGIA(end);
            mbaterias_descenso=(PESO(end));
            ...
            fuel_descenso = (Newt(1)-Newt(end))/g;
            energia_descenso=0;
            mbaterias_descenso=0;
            datos(i).segmento.fuel = abs((Newt(1) - Newt)./g); %fuel consumido acumulado
    ...
    case 3
        ...
        if tipo_motor==4
            % dEdh= P_sol./(Vvect'*sin(gamma).*eta_mp(nps_sol));
            dEdh= P_sol./(V*sin(gamma).*eta_mp(nps_sol)*eta_m); %dEdh=dEdx*dxdh=dEdt*dxdt*1/tan(gamma)=P/(V*tan(gamma))
            ENERGIA=dEdh(:).*(vector_h(:)-h_ini);
            PESO=ENERGIA/e0;
            Newt=PESO*g;
    ...
    case 4
        ...
        datos(i).segmento.gamma = gamma(1:end-1);
    ...
    case 8
        ...
        for k = 1:(n-1),    %PARA CADA ALTURA DESDE H_INICIAL A H_FINAL
            ...
            % Aqui ya tendríamos CL,CD, la palanca que hace el gamma max para el h y
            % w dado
            L_D(k) = CL/CD;
            CL2(k) = CL;
            CD2(k) = CD;
