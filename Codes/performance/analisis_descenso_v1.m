function [fuel_descenso,tiempo_descenso,S_descenso,datos] = analisis_descenso(propul,aerodinamica,descenso,W_inicial,h_inicial,opcion,i,datos)
...
EAS = descenso(4);
TAS = descenso(5);
delta_T_gases = descenso(6);
velocidad_ini = descenso(7);
velocidad_fin = descenso(8);
...
switch opcion
    case 1
%DESCENSO DADO GAMMA = CONSTANTE, MACH = CONSTANTE, ALTURA FINAL
M = Mach;
gamma = angulo;
V = @(h) M .* a(h);
...
% if palanca(k) > 1,
%     tope = 'Palanca de gases superior a la unidad';
%     h_final = H(k);
%     Newt(end) = Newt(k);
%     k = n+1;
% else
...
tiempo_descenso = tiempo(h_final);
S_descenso = (h_final-h_ini)/(tan(gamma));
fuel_descenso = (Newt(1)-Newt(end))/g;
W = Newt(end);

datos(i).descenso.tiempo = (H-H(1))./V_v';
datos(i).descenso.fuel = abs((Newt(1) - Newt)./g); %fuel consumido acumulado
datos(i).descenso.distancia = (H-H(1))./tan(gamma);
datos(i).descenso.palanca = palanca;
%datos(i).descenso.empuje = empuje;
datos(i).crucero.empuje = CL2;
datos(i).descenso.veloc_vertical = V_v;
datos(i).descenso.L_D = L_D;
datos(i).descenso.peso = Newt;
% datos(i).descenso.tope = tope;
datos(i).descenso.gamma = gamma; %%% ESCALAR %%%
datos(i).descenso.velocidad = velocidad;
datos(i).descenso.altura = H;
...
%-------------------------------------------------------------------------%
    ...
    case 2
% descenso DADO GAMMA = CONSTANTE, EAS = CONSTANTE, ALTURA FINAL
...
%-------------------------------------------------------------------------%
    ...
    case 3
% descenso DADO GAMMA = CONSTANTE, V = TAS = CONSTANTE, ALTURA FINAL
gamma = angulo;
V = @(h) TAS;
M = @(h) V(h)./a(h);
...
tiempo_descenso = tiempo(h_final);
S_descenso = (h_final-h_ini)/(tan(gamma));
fuel_descenso = abs((Newt(1)-Newt(end))/g);
W = Newt(end);
...
%-------------------------------------------------------------------------%
    ...
    case 4
% descenso DADOS PALANCA DE GASES = CONSTANTE, MACH = CONSTANTE, ALTURA FINAL
M = Mach;
...
while contador <= 1000,
   ...
   empuje(k) = abs(T(H(k)));
   fuel(k) = abs(C(H(k))*T(H(k))*tiempo(k));
   w(k+1) = w(k) - fuel(k) * g;      %Calculamos el nuevo peso que tendra la siguiente h
   gamma(k+1) = gamma(k);
end

tiempo_descenso = sum(tiempo);
S_descenso = sum(distancia);
fuel_descenso = sum(fuel);
W = W_ini - fuel_descenso*g;

datos(i).descenso.tiempo = cumsum(tiempo);
datos(i).descenso.fuel = cumsum(fuel); %fuel consumido acumulado
datos(i).descenso.distancia = cumsum(distancia);
datos(i).descenso.palanca = delta_T;%%% ESCALAR %%%
%datos(i).descenso.empuje = empuje;
datos(i).descenso.empuje = CL2;
datos(i).descenso.veloc_vertical = V_v;
datos(i).descenso.L_D = L_D; %%% ESCALAR %%%
datos(i).descenso.peso = w(1:end-1);
datos(i).descenso.gamma = gamma(1:end-1);
%datos(i).descenso.techo = techo;
datos(i).descenso.velocidad = velocidad;
datos(i).descenso.altura = H(1:end-1);
%-------------------------------------------------------------------------%
%-------------------------------------------------------------------------%
%-------------------------------------------------------------------------%
%-------------------------------------------------------------------------%
%-------------------------------------------------------------------------%
 ...
 case 5
% descenso DADOS PALANCA DE GASES = CONSTANTE, EAS = CONSTANTE, ALTURA FINAL
delta_T = delta_T_gases;
...
case 6
% descenso DADOS PALANCA DE GASES = CONSTANTE, TAS = CONSTANTE, ALTURA FINAL
delta_T = delta_T_gases;
...
%-------------------------------------------------------------------------%
%-------------------------------------------------------------------------%
% DESCENSO ACELERADO LINEALMENTE DADOS GAMMA Y LA VELOCIDAD INICIAL Y FINAL (TAS)
...
switch tipo_motor
    case 1
        delta_T = @(h,W) T(h,W)./(T_SL.*(1+0.2.*M(h).^2).^(1.4./0.4).*(1-0.49.*sqrt(M(h))).*(rho(h)./rho_SL));
        correccion = @(h,W) abs((a_1.*delta_T(h,W).^4+a_2.*delta_T(h,W).^3+a_3.*delta_T(h,W).^2+a_4.*delta_T(h,W)+a_5));
        switch derivacion
            case 1
                C = @(h,W) c_SL.*correccion(h,W).*(1+1.2.*M(h)).*sqrt(Temp(h)./Temp_SL);
            case 2
...
L_D(k) = CL(H(k),Newt(k))./CD(H(k),Newt(k));
CL2(k) = CL(H(k),Newt(k));
% if palanca(k) < 0 || palanca(k) > 1
%     tope = 'Palanca de gases fuera de los limites permitidos';
%     k = n+1;
% else
%     tope = 'Palanca de gases correcta';
% end
k = k + 1;
end
...
%-------------------------------------------------------------------------%
%-------------------------------------------------------------------------%
% DESCENSO MINIMO GAMMA DADO PALANCA DE GASES Y ALTURA FINAL
...
datos(i).descenso.tiempo = cumsum(tiempo);
datos(i).descenso.fuel = cumsum(fuel);
datos(i).descenso.distancia = cumsum(distancia);
datos(i).descenso.palanca = delta_T;
datos(i).descenso.empuje = empuje;
datos(i).descenso.veloc_vertical = V_v;
datos(i).descenso.L_D = L_D;
datos(i).descenso.peso = w(1:end-1);
%datos(i).subida.techo = techo;
datos(i).descenso.gamma = gam;
datos(i).descenso.velocidad = velocidad;
datos(i).descenso.altura = H(1:end-1);
%-------------------------------------------------------------------------%
%-------------------------------------------------------------------------%
...
% descenso SLOWEST CLIMB DADO PALANCA DE GASES Y ALTURA FINAL
...
while contador <= 1000,
   ...
   if abs(gamma(ii) - gamma_obt) <= 10^-6,
       contador = 1001;
   end
   contador = contador + 1;
   gamma(ii) = gamma_obt;
...
ii = ii + 1;
if ii > 2,
if abs(veloc_vert(ii-1)-veloc_vert(ii-2)) <= 10^-4, break; end;
end
...
%-------------------------------------------------------------------------%
 ...
 case 10
% DESCENSO DADOS PALANCA DE GASES = CONSTANTE, VELOCIDAD INICIAL Y FINAL
Vf = velocidad_fin;
V0 = velocidad_ini;
delta_T = delta_T_gases;
...
while contador <= 1000,
   ...
   V_v(k) = V(H(k)) * sin(gamma(k));
   velocidad(k) = V(H(k)) * cos(gamma(k));
   tiempo(k) = abs((H(k+1)-H(k))/V_v(k));
   distancia(k) = abs((H(k+1)-H(k))/tan(gamma(k)));
   empuje(k) = T(H(k));
   fuel(k) = C(H(k))*T(H(k))*tiempo(k);
   w(k+1) = w(k) - fuel(k) * g;      %Calculamos el nuevo peso que tendra la siguiente h
   gamma(k+1) = gamma(k);
...
datos(i).nombre = 'Descenso';
datos(i).lista_variables = [{''};'Tiempo';'Fuel';'Peso';'Distancia';'Angulo de descenso';'Velocidad';'Velocidad vertical';'Palanca';'CL';...
    'L/D';'Altura'];

global flag_palanca 
if max(datos(i).descenso.palanca) > 1.2,
    flag_palanca = 1;
else
    flag_palanca = 0;
end
