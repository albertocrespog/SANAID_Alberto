function [fuel_crucero,mbaterias_crucero,energia_crucero,tiempo_crucero,distancia_crucero,datos] = analisis_crucero_v3(propul,aerodinamica,crucero,W_inicial,h_inicial,opcion,datos,i,Geo_tier,AC_CONFIGURATION,Prop_data,data_electric,pesos)
% PROPUL
...
% Dprop=data_electric(2)*0.0254; %from inches to m
...
k1_polar = crucero(9);
k2_polar = crucero(10);
mbat_nominal = crucero(11); %Masa EN BRUTO de baterías
payload = pesos(2);
empty_weight = pesos(1);
...
switch opcion
    ...
    case 1  %CRUCERO A MACH CONSTANTE Y DISTANCIA DADA
        ...
        % Selects integration according to the engine model
        if tipo_motor == 1 || tipo_motor == 2 || tipo_motor == 3 || tipo_motor == 5
            ...
            datos(i).segmento.fuel   = (PESO(1) - PESO)./g;
            datos(i).segmento.mbaterias = 0;
            datos(i).segmento.energia = 0;
            datos(i).segmento.fuel(end) = '';
            datos(i).segmento.distancia = X_v(1:end-1);
            ...
            datos(i).segmento.velocidad = V;%escalar
            datos(i).segmento.CD = CD2;
            
        else
            % for electric engine with constant weight
            dEdx = @(x,nps) P_sol/V; %dE/dx*dt/dt=dE/dt*dt/dx
            [X,ENERGIA] = ode45(dEdx,X_v,E_ini);
%             PESO=ones(size(ENERGIA))*(ENERGIA(end)/(etamp_sol*eta_m))/(e0)*g; %Transforms energy needed for performance to energy substracted from batteries
            PESO=ones(size(ENERGIA))*W_ini;
            for k = 1:(num-1)
                peso(k) = PESO(k);
                energia(k)=ENERGIA(k);
                L_D(k) = CL(PESO(k))/CD(PESO(k));
...
%             fuel_crucero = (peso(1)-peso(end))/g;
            fuel_crucero = 0;
            mbaterias_crucero = (energia(end)-energia(1))/(e0*etamp_sol*eta_m);
            energia_crucero = (energia(end)-energia(1)/(etamp_sol*eta_m));
            tiempo_crucero = sum(tiempo);
            distancia_crucero = dist_final;
            W = peso(end);
...
%             datos(i).segmento.mbaterias = (energia(end)-energia(1))/(e0*etamp_sol*eta_m);
%             datos(i).segmento.energia = energia(end)-energia(1);
%             datos(i).segmento.energiav = energia;
            datos(i).segmento.mbaterias = (energia-energia(1))/(e0*etamp_sol*eta_m);
            datos(i).segmento.energia = energia(end)-energia(1)/(etamp_sol*eta_m);
            datos(i).segmento.energiav = (energia-energia(1))/(etamp_sol*eta_m);
            datos(i).segmento.fuel(end) = '';
            datos(i).segmento.distancia = X_v(1:end-1);
            datos(i).segmento.palanca = palanca;
            datos(i).segmento.empuje = empuje;
            datos(i).segmento.CL = CL2;
            datos(i).segmento.L_D = L_D;
            datos(i).segmento.peso = W_ini*ones(1,length(datos(i).segmento.tiempo));
            datos(i).segmento.velocidad = V;%escalar
            datos(i).segmento.CD = CD2;
        end
        ...
        try
            ...
            datos(i).segmento.Vmax = Vmax;
        catch
            datos(i).segmento.Vmax = 0;
        end
        %-------------------------------------------------------------------------%
...
%             datos(i).segmento.energia = energia(end)-energia(1);
    ...
    case 3   %CRUCERO ACELERADO CON VELOCIDAD INICIAL Y FINAL DADOS Y ALTURA DADA Y PALANCA DE GASES DADA
        ...
        if tipo_motor==4
            ...
            dEdv= @(V,nps) W_ini*P(V)./((T(V)-D(V)).*etamp(V)*eta_m*eta_m)/g;
        ...
        datos(i).segmento.tiempo = cumsum(tiempo);
        if tipo_motor == 4
            datos(i).segmento.mbaterias = (energia(end)-energia(1))/(e0);
            datos(i).segmento.energia = energia(end)-energia(1);
        else
            datos(i).segmento.mbaterias = 0;
            datos(i).segmento.energia = 0;
        ...
        datos(i).segmento.L_D = L_D;
        datos(i).segmento.peso = peso;
        datos(i).segmento.velocidad = VEL;
    ...
    case 5 %CRUCERO DE MAXIMO ALCANCE DADO WFINAL
        ...
        datos(i).segmento.mbaterias = 0;
        datos(i).segmento.energia = 0;
        datos(i).segmento.distancia = distancia;
        datos(i).segmento.palanca = palanca;
        datos(i).segmento.empuje = empuje;
        datos(i).segmento.CL = CL2;
        datos(i).segmento.CD = CD2;
    ...
    case 7 %Crucero dada Mach de vuelo y masa de baterías a usar
        ...
        %         V = 27.9;           %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%ELIMINAR ESTO EN LA VERSIÓN PROGRAMADA%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        
        mbat_util = mbat_nominal*(1-tau);
        % ENERGIA = mbat_util*e0-110952;
        % Corrección pr qeu no se por que está - 110952
        ENERGIA = mbat_util*e0;
        ...
        datos(i).segmento.tiempo = tiempo_crucero;
        datos(i).segmento.fuel   = fuel_crucero;
        datos(i).segmento.mbaterias = mbaterias_crucero;
        datos(i).segmento.energia= energia_crucero;
        datos(i).segmento.distancia = rango;

        % Engine properties
        datos(i).segmento.palanca = delta_T;
        datos(i).segmento.empuje = T(nps_sol);
        datos(i).segmento.P_sol = P_sol; % POWER
        datos(i).segmento.velocidad = V;%escalar
        datos(i).segmento.etamp = etamp_sol;
        
        % aERODYNAMICS
        datos(i).segmento.CL = CL_sol;
        datos(i).segmento.CD = CD_sol;
        datos(i).segmento.L_D = L_D;
...
global flag_palanca
if max(datos(i).segmento.palanca) > 10
    flag_palanca = 1;
    return;
else
    flag_palanca = 0;
