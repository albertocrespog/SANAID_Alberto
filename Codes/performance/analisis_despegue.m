function [fuel_total,tiempo_total,S_total,datos] = analisis_despegue(despegue_vec,propul,aerodinamica,aero_despegue,W_inicial,datos,i,Prop_data,data_electric)
...
despegue(1) = despegue_vec.temp_local;
despegue(2) = despegue_vec.h_inicial;
despegue(3) = despegue_vec.P_local;
despegue(4) = despegue_vec.mu_takeoff;
despegue(5) = despegue_vec.h_obstacle;
despegue(6) = despegue_vec.gamma_climb;
despegue(7) = despegue_vec.delta_T;
...
datos(i).segmento.transicion.tiempo = tiempo_tr;
datos(i).segmento.transicion.distancia = S_tr;
datos(i).segmento.transicion.fuel = fuel_tr;
datos(i).segmento.transicion.velocidad = V_tr;
datos(i).segmento.transicion.altura = h_tr;
datos(i).segmento.transicion.gamma = gamma_subida;

datos(i).segmento.climb.tiempo = tiempo_c;
datos(i).segmento.climb.distancia = S_c;
datos(i).segmento.climb.fuel = fuel_c;
datos(i).segmento.climb.velocidad = V_c;
datos(i).segmento.climb.altura = h_obst;
datos(i).segmento.climb.detalle = h_tr_muy_elevada;

datos(i).segmento.tiempo = tiempo_total;
datos(i).segmento.distancia = S_total;
datos(i).segmento.fuel = fuel_total;
