function [fuel_viraje,tiempo_viraje,distancia_viraje,datos] = analisis_viraje(propul,aerodinamica,viraje,W_inicial,h_inicial,opcion,i,datos)
...
datos(i).viraje.tiempo = linspace(0,tiempo_final,num);
datos(i).viraje.fuel = (peso(1) - peso)./g; %fuel consumido acumulado
datos(i).viraje.vueltas = vueltas; %escalar
datos(i).viraje.palanca = delta_T;
% datos(i).viraje.empuje = empuje;
datos(i).viraje.empuje = CL2;
datos(i).viraje.L_D = L_D;
datos(i).viraje.peso = peso;
datos(i).viraje.velocidad = V;
datos(i).viraje.alabeo = alabeo;
datos(i).viraje.factor_de_carga = n;
datos(i).viraje.velocidad_guiniada = veloc_chi;
datos(i).viraje.radio_giro = radio_giro;
...
datos(i).viraje.tiempo = TT_v;
datos(i).viraje.fuel = (peso(1) - peso)./g; %fuel consumido acumulado
datos(i).viraje.vueltas = vueltas; %escalar
datos(i).viraje.palanca = palanca;
%datos(i).viraje.empuje = empuje;
datos(i).viraje.empuje = CL2;
datos(i).viraje.L_D = L_D;
datos(i).viraje.peso = peso;
...
datos(i).viraje.tiempo = TT;
datos(i).viraje.tiempo(end) = '';
datos(i).viraje.fuel = (w(1) - w)./g; %fuel consumido acumulado
datos(i).viraje.fuel(end) = '';
datos(i).viraje.vueltas = vueltas; %escalar
datos(i).viraje.palanca = palanca;
% datos(i).viraje.empuje = empuje;
datos(i).viraje.empuje = CL2;
datos(i).viraje.L_D = L_D;
datos(i).viraje.peso = w;
datos(i).viraje.peso(end) = '';
datos(i).viraje.velocidad = v;
datos(i).viraje.alabeo = alabeo;
datos(i).viraje.factor_de_carga = n;
datos(i).viraje.velocidad_guiniada = veloc_chi;
...
datos(i).viraje.altura = h_inicial;
datos(i).viraje.distancia = 0;
