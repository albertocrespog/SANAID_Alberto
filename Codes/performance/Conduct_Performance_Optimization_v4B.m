function Storing_PERFORMANCE_DATA_2B = Conduct_Performance_Optimization_v4B(AC_CONFIGURATION,case_AC,Propulsion,Prop_data,conv_UNITS,...
...
for i=1:length(m_fuel_VAR)
    ...
    %     OUTPUT_read_XLSX.IPP_flags.V_cr = V_VAR(j);
        ...
        % Calculations
        Storing_PERFORMANCE_2{i} = Conduct_Performance_Optimization_v2(AC_CONFIGURATION,case_AC,Propulsion,Prop_data,conv_UNITS,...
...
if OUTPUT_read_XLSX.MATLAB_flags.save_MAT == 1
    Storing_PERFORMANCE_DATA_2B = Saving_data_Performance_varV_M(Storing_PERFORMANCE_2,Plot_Options,Restrictions_var_V_m,OUTPUT_read_XLSX,Storing_PROPULSION_DATA,filenameS);

else
    % Stores dummy variable for continuity
    dummy = 1;
    Storing_PERFORMANCE_DATA_2B = dummy;
end