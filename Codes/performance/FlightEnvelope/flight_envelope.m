...
for i = 1:length(h)
    ...
    try
        V_max_endurance(i) = fmincon(@(V) 1e4*cruise_fuel_consumption(V,h(i),S,C_D0,C_D1,C_D2,m,g),x0,[],[],[],[],x_A(1),x_C(1),[],misops);
    catch
        h(i:end)=[];
        break
    end
...
V_stall = sqrt(m*g./(0.5*rho*C_L_max*S));
V_min = 1.3*V_stall;
V_max_endurance = fmincon(@(V) 1e4*cruise_fuel_consumption(V,h_1,S,C_D0,C_D1,C_D2,m,g),V_max_endurance(end),[],[],[],[],x_A(1),x_C(1),[],misops);
