function Storing_PERFORMANCE_DATA_4 = Conduct_Performance_Optimization_v6(AC_CONFIGURATION,case_AC,Propulsion,Prop_data,conv_UNITS,...
...
for i=1:length(V_VAR)
    ...
    for j=1:length(h_VAR)
        ...
        % Calculations
        Storing_PERFORMANCE_4{i,j} = Conduct_Performance_Optimization_v2(AC_CONFIGURATION,case_AC,Propulsion,Prop_data,conv_UNITS,...
...
if OUTPUT_read_XLSX.MATLAB_flags.save_MAT == 1
    Storing_PERFORMANCE_DATA_4 = Saving_data_Performance_var_h_V_Glide(Storing_PERFORMANCE_4,Plot_Options,Restrictions_var_h_V,OUTPUT_read_XLSX,Storing_PROPULSION_DATA,filenameS);

else
    % Stores dummy variable for continuity
    dummy = 1;
    Storing_PERFORMANCE_DATA_4 = dummy;
end