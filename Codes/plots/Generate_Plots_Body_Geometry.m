function [Fig,Body_Geo,plotData] = Generate_Plots_Body_Geometry(XFLR5_file,STL_files,Geo_tier,...
    nPoints, nSections,conversion_UNITS,lecture_Geo,Ratio_SCALE,SF,OUTPUT_read_XLSX,Fig,filenameS)
...
if OUTPUT_read_XLSX.Fuselage_flags.Use_Storing_DATA == 0
    ...
    Saving_data_STL_SLICER(original, movelist, z_slices_tmp,OUTPUT_read_XLSX,filenameS);
    % % Plots
    % plot_slices(movelist,z_slices_tmp, 0)

else
    [original, movelist, z_slices_tmp] = Read_data_STL_SLICER(OUTPUT_read_XLSX,filenameS);
end
...
%% Asigns the values obtained from STL file
for i=1:length(movelist)
    % construct the first point using the firs value of the slices and
    % reduce it
    % Eliminates NaN
    Y = movelist{i}(:,2)*SF_CAD;
    Y = Y';
    Y = Y(~isnan(Y))';
    Y_stl{i+1}(1,:) = Y;
    % Eliminates NaN
    Z = movelist{i}(:,1)*SF_CAD;
    Z = Z';
    Z = Z(~isnan(Z))';
    Z_stl{i+1}(1,:) = Z;
    J = length(movelist{i});
...
Body_Geo.Delta_Y_stl = Delta_Y_stl;
Body_Geo.Delta_Z_stl = Delta_Z_stl;
...
plot_3D = OUTPUT_read_XLSX.PLOT_flags.plot_3D;
if plot_3D==1
    if PLOT_Slices_fuselage_STL == 1
        ...
        grid on
        title('Fuselage rotated around the y-axis to be sliced (original STL)')
        ...
        title('Fuselage y-axis vs z-axis - Not Shifted (original STL - Scaled)')
        ...
        title('Fuselage y-axis vs z-axis - Shifted to origin (@ the nose) & Scaled')
        xlabel('Fuselage y-axis')
        ylabel('Fuselage z-axis')
        grid on
        hold off
...
if plot_3D==1
    if PLOT_Slices_fuselage_STL == 1
        ...
        grid on
        title('Fuselage Perimeter persections (method 2)')
        xlabel('Fuselage x-axis sections')
        ylabel('Fuselage Area per section')
...
for i=2:(length(z_slices)-1)
    z_slices_surface(i) = z_slices(i);
    Delta_h = x_max(i) - x_max(i-1);
    % if i==2
        % Vol(i) = Delta_h*(Area_body(i) + Area_body0)/2;
    % else
        Vol(i) = Delta_h*(Area_body(i) + Area_body(i-1))/2;
    % end        
    % l = l+1;
end
