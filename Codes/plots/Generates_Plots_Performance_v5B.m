%%%%%% PRUEBA PLOTS %%%%%%
function [Fig] = Generates_Plots_Performance_v5B(Storing_PERFORMANCE_DATA,Plot_Options,OUTPUT_read_XLSX,Restrictions,Fig,filenameS)
%close all
...
for i=1:length(V_VAR)
    for j=1:length(h_VAR)
        ...
        Time_PLOTS(i,j) = Storing_PERFORMANCE_DATA.Storing_PERFORMANCE_23{i,j}.Total_Datos.tiempo_total/60;
        Distance_PLOTS(i,j) = Storing_PERFORMANCE_DATA.Storing_PERFORMANCE_23{i,j}.Total_Datos.distancia_total/1000;
        Wempty_PLOTS(i,j) = Storing_PERFORMANCE_DATA.Storing_PERFORMANCE_23{i,j}.Weights_AP.m_TOW;
        W_final_PLOTS(i,j) = Storing_PERFORMANCE_DATA.Storing_PERFORMANCE_23{i,j}.Weights_AP.m_F;
        fuel_PLOTS(i,j) = Storing_PERFORMANCE_DATA.Storing_PERFORMANCE_23{i,j}.Weights_AP.m_f;
...
figure(Fig)
% subplot (1,2,1)
[x1 y1] = contour(V_VAR_MAT,h_VAR_MAT,fuel_PLOTS,[0 0]); % x1 will be your "two-column matrix"
V_alpha_cero = x1(2,2:end);
M_alpha_cero = x1(1,2:end);
surf(h_VAR,V_VAR,fuel_PLOTS)
shading interp
colormap(jet) % colors paletts for the 3-D graph if you type help graph3d will give you a list of different color types
colorbar % adds the color bar to the right of the graph
%like hot, or graybone, copper, pink, jet (the one used here) spring, winter, summer, autumn etc...
%    rotate3d on % allows you to rotate the graph with the mouse
colorbar % adds the color bar to the right of the graph
title('Fuel (kg) vs V and Altitude','FontSize',FS)
xlabel('Altitude (m)','FontSize',FS)
ylabel('Velocity (m/s)','FontSize',FS)
zlabel('Fuel (kg)','FontSize',FS)
if SAVE_FIGS==1
    prefix = strcat(OUTPUT_read_XLSX.PLOT_flags.prefix);
    st = strcat('fuel_mission_vs_h&V');
    name   = strcat(prefix,st);
    % fname = OUTPUT_read_XLSX.PLOT_flags.fname;
    % Unified plot Options
    SAVE_types(fname,name,gca,gcf);
...
figure(Fig)
N_contour_lines = 20; % Number of contour lines
vect_cc_time_mission_plot = linspace(min(min(fuel_PLOTS')),max(max(fuel_PLOTS')),N_contour_lines);
% [alpha_c,h_alpha_c] = contourf(V_VAR,m_bat_VAR,Time_PLOTS,vect_cc_time_mission_plot');
[alpha_c,h_alpha_c] = contourf(h_VAR,V_VAR,fuel_PLOTS,vect_cc_time_mission_plot');
clabel(alpha_c,h_alpha_c)
hold on
plot(h_VAR,V_stall(1,:),'m--','LineWidth',LS)
plot(h_VAR,V_TO(1,:),'o-.','LineWidth',LS)
...
colorbar % adds the color bar to the right of the graph
grid on
Title = strcat('Fuel (kg) vs V and Altitude');
title(Title,'FontSize',FS)
xlabel('Altitude (m)','FontSize',FS)
ylabel('Velocity (m/s)','FontSize',FS)
zlabel('Fuel (kg)','FontSize',FS)
...
if SAVE_FIGS==1
    prefix = strcat(OUTPUT_read_XLSX.PLOT_flags.prefix);
    st = strcat('fuel_mission_vs_h&V_contour');
    name   = strcat(prefix,st);
    % fname = OUTPUT_read_XLSX.PLOT_flags.fname;
            % Unified plot Options             
            SAVE_types(fname,name,gca,gcf); 
