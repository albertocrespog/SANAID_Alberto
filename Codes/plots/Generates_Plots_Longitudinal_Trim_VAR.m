function [Fig,M_alpha_cero,V_alpha_cero] = Generates_Plots_Longitudinal_Trim_VAR(TRIM_RESULTS_var,Trim_ITER,Restrictions_var_V_XCG,...
    Stab_Der_var_V_XCG,Geo_tier,Plot_Options,Fig,OUTPUT_read_XLSX,x_XCG_VAR,filenameS)
...
figure(Fig)
% subplot (1,2,1)
[x1 y1] = contour(m_VAR_MAT,V_VAR_MAT,trim_alpha_deg_plot,[0 0]); % x1 will be your "two-column matrix"
V_alpha_cero = x1(2,2:end);
M_alpha_cero = x1(1,2:end);
surf(m_VAR,V_VAR,trim_alpha_deg_plot)
shading interp
...
[alpha_c,h_alpha_c] = contourf(m_VAR,V_VAR,trim_alpha_deg_plot,vect_cc_trim_alpha_deg_plot');
clabel(alpha_c,h_alpha_c)
hold on
plot(m_VAR,V_stall(1,:),'m--','LineWidth',LS)
plot(m_VAR,V_TO(1,:),'o-.','LineWidth',LS)
plot(m_VAR,V_min_ope(1,:),'^:','LineWidth',LS)
...
if SAVE_FIGS==1
    prefix = strcat(OUTPUT_read_XLSX.PLOT_flags.prefix);
    st = strcat('alpha_vs_V&m_contour');
    name   = strcat(prefix,st);
    % fname = OUTPUT_read_XLSX.PLOT_flags.fname;
...
figure(Fig)
% subplot (1,2,1)
surf(m_VAR,V_VAR,trim_delta_e_deg_plot)
shading interp
colormap(jet) % colors paletts for the 3-D graph if you type help graph3d will give you a list of different color types
colorbar % adds the color bar to the right of the graph
...
[delta_c,h_delta_c] = contourf(m_VAR,V_VAR,trim_delta_e_deg_plot,vect_cc_trim_delta_deg_plot');
...
if SAVE_FIGS==1
    prefix = strcat(OUTPUT_read_XLSX.PLOT_flags.prefix);
    st = strcat('delta_e_vs_V&m_contour');
    name   = strcat(prefix,st);
    % fname = OUTPUT_read_XLSX.PLOT_flags.fname;
            % Unified plot Options             
            SAVE_types(fname,name,gca,gcf); 
...
[deltaT_c,h_deltaT_c] = contourf(m_VAR,V_VAR,trim_delta_T,vect_cc_trim_deltaT_plot');
...
Fig = Fig + 1;
    ...
    [delta_SM_c,h_XCG_c] = contourf(m_VAR,V_VAR,SM*100,vect_cc_trim_SM_plot');
...
if isConstantZData
    ...
    figure(Fig)
    % subplot (1,2,1)
    surf(m_VAR,V_VAR,trim_CM_alpha_ac)
    shading interp
    colormap(jet) % colors paletts for the 3-D graph if you type help graph3d will give you a list of different color types
    colorbar % adds the color bar to the right of the graph
    ...
    [delta_CMalpha_c,h_CMalpha_c] = contourf(m_VAR,V_VAR,trim_CM_alpha_ac,vect_cc_trim_CMalpha_plot');
    ...
    if SAVE_FIGS==1
        prefix = strcat(OUTPUT_read_XLSX.PLOT_flags.prefix);
        st = strcat('CMalpha_vs_V&m_contour');
        name   = strcat(prefix,st);
        % fname = OUTPUT_read_XLSX.PLOT_flags.fname;
        % Unified plot Options
        SAVE_types(fname,name,gca,gcf);
...
if isConstantZData
    ...
    [delta_CM0_c,h_CM0_c] = contourf(m_VAR,V_VAR,trim_CM0_ac,vect_cc_trim_CM0_plot');
...
[delta_CL_w1_c,h_CL_w1_c] = contourf(m_VAR,V_VAR,CL_w1,vect_cc_trim_CL_w1_plot');
...
% [delta_CL_HTP_c,h_CL_HTP_c] = contourf(m_VAR,V_VAR,CL_HTP,vect_cc_trim_CL_HTP_plot');
...
[delta_CL_needed_c,h_CL_needed_c] = contourf(m_VAR,V_VAR,CL_needed,vect_cc_trim_CL_needed_plot');
...
if SAVE_FIGS==1
    prefix = strcat(OUTPUT_read_XLSX.PLOT_flags.prefix);
    st = strcat('CLneeded_vs_V&m_contour');
    name   = strcat(prefix,st);
    % fname = OUTPUT_read_XLSX.PLOT_flags.fname;
            % Unified plot Options             
            SAVE_types(fname,name,gca,gcf); 
...
figure(Fig)
% subplot (1,2,1)
surf(m_VAR,V_VAR,x_XCG_MAT)
shading interp
colormap(jet) % colors paletts for the 3-D graph if you type help graph3d will give you a list of different color types
colorbar % adds the color bar to the right of the graph
title('$x_{CG}$ (m) vs V and mass','FontSize',FS)
ylabel('Velocity  (m/s)','FontSize',FS)
xlabel('mass (kg)','FontSize',FS)
zlabel('x_{CG} (m)','FontSize',FS)
if SAVE_FIGS==1
    prefix = strcat(OUTPUT_read_XLSX.PLOT_flags.prefix);
    st = strcat('XCG_vs_V&m');
    name   = strcat(prefix,st);
    % fname = OUTPUT_read_XLSX.PLOT_flags.fname;
            % Unified plot Options             
            SAVE_types(fname,name,gca,gcf); 
...
Fig = Fig + 1;
    ...
    [delta_XCG_c,h_XCG_c] = contourf(m_VAR,V_VAR,x_XCG_MAT,vect_cc_trim_XCG_plot');
    ...
    colorbar % adds the color bar to the right of the graph
    legend('\alpha(V,m)','V_{stall}','1.2 V_{stall}','1.3 V_{stall}','V_{\alpha=0^\circ}','Location','northwest','FontSize',LFS)
    title('$x_{CG}$ vs V and mass','FontSize',FS)
    ylabel('Velocity  (m/s)','FontSize',FS)
    xlabel('mass (kg)','FontSize',FS)
