...
sum_masas=sum_masas_STR+TAIL(m)+MSL(m)+PLD(m)+GAS(m)+SYS(m)+MAZ_ELE(m)+W_PEG_TOR;
...
x_cg=((sum_masas_STR+W_PEG_TOR)*x_cg_STR + TAIL(m)*TAIL(x) + MSL(m)*MSL(x) + PLD(m)*PLD(x) + GAS(m)*GAS(x)+ SYS(m)*SYS(x))/sum_masas;
...
y_cg=((sum_masas_STR+W_PEG_TOR)*y_cg_STR + TAIL(m)*TAIL(y) + MSL(m)*MSL(y) + PLD(m)*PLD(y) + GAS(m)*GAS(y)+ SYS(m)*SYS(y))/sum_masas;
...
z_cg=((sum_masas_STR+W_PEG_TOR)*z_cg_STR + TAIL(m)*TAIL(z) + MSL(m)*MSL(z) + PLD(m)*PLD(z) + GAS(m)*GAS(z)+ SYS(m)*SYS(z))/sum_masas;



x_morro=540.224; % distancia antiguo del mamparo al morro
...
z_cg_STR=z_cg_STR*0.001;
 ...
 I_PEG_TOR(1) =  +W_PEG_TOR*(y_cg_STR^2+z_cg_STR^2);
I_PEG_TOR(2)=  + W_PEG_TOR*(x_cg_STR^2+z_cg_STR^2);
I_PEG_TOR(3) =   + W_PEG_TOR*(x_cg_STR^2+y_cg_STR^2);
I_PEG_TOR(4) =   - W_PEG_TOR*x_cg_STR*y_cg_STR;
I_PEG_TOR(5) =  - W_PEG_TOR*x_cg_STR*z_cg_STR;
I_PEG_TOR(6)=   - W_PEG_TOR*y_cg_STR*z_cg_STR;
 

I_TOTAL = FUS(5:10) + NSE_LGR(5:10) + PPL_LGR(5:10) + MPS(5:10)  + TAIL(5:10)...
   + WNG_RGT(5:10) + WNG_LFT(5:10) + RACK_LFT(5:10) + RACK_RGT(5:10) + MSL(5:10) + PLD(5:10) + GAS(5:10) + SYS(5:10)+MAZ_ELE(5:10)+I_PEG_TOR;
