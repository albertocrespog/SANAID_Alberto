function [percentage_FUEL OUTPUT] =  CG_TARSIS120_KSA_V6B(W_PL, T120, excel_FUEL, caso,porcentaje_actual)
...
elseif caso == 3 % modified to be consisten con old forumation
    % BAT no presente
    fuel_max_real = min(20.327, 120 - (FUS(1) + WNG_LFT(1) + WNG_RGT(1) + VTP_LFT(1) + VTP_RGT(1) + HTP(1) + KSA_CPR(1) + PLD(1)));

elseif caso == 4 % modified to be consisten con old forumation
    % BAT sí presente
...
MTOW_fuel = fuel_max_real*porcentaje_actual;
