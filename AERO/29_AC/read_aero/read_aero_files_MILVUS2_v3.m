...
% New Wing for only wing configuration
casos{casE} = 'wing_extended2_T1-25_0 m_s-TriUniform-ThickSurf-x0_029m.txt'; 
X_OC(casE) = 0.029;
st{casE} = strcat('TRI - W_1 (',num2str(V), 'm/s) @',num2str(h),' m X_{ac} =',num2str(X_OC(casE)),' m');
casE = casE + 1;

% Front Wing analysis SideSlip (same as front)
casos{casE} = 'wing_extended2_T5-a0_0°-25_0m_s-TriUniform-ThickSurf-x0_029m.txt'; 
X_OC(casE) = 0.029;
st{casE} = strcat('TRI - W_1 (',num2str(V), 'm/s) @',num2str(h),' m X_{ac} =',num2str(X_OC(casE)),' m');
casE = casE + 1;
